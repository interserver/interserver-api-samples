...
namespace IO.Swagger.Api
{
    ...
    public class ServersApi : IServersApi
    {
        ...
        public InlineResponse20021 AddServer (ServerOrderPostRequest body)
        {
            ...
            if (body == null) throw new ApiException(400, "Missing required parameter 'body' when calling AddServer");
        ...
        public ServerBulkIpmiPowerResponse ServerBulkIpmiPowerGet (string ids)
        {
            ...
            if (ids == null) throw new ApiException(400, "Missing required parameter 'ids' when calling ServerBulkIpmiPowerGet");
    
            var path = "/servers/bulk/ipmi_power";
            path = path.Replace("{format}", "json");
            ...
            if (((int)response.StatusCode) >= 400)
                throw new ApiException ((int)response.StatusCode, "Error calling ServerBulkIpmiPowerGet: " + response.Content, response.Content);
            else if (((int)response.StatusCode) == 0)
                throw new ApiException ((int)response.StatusCode, "Error calling ServerBulkIpmiPowerGet: " + response.ErrorMessage, response.ErrorMessage);
    
            return (ServerBulkIpmiPowerResponse) ApiClient.Deserialize(response.Content, typeof(ServerBulkIpmiPowerResponse), response.Headers);
        }
