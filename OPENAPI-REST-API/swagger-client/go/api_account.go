...
/*
AccountApiService Unlink a third-party OAuth/social provider (Google, GitHub, etc.) from the account
...
/*
AccountApiService Disable two-factor authentication and remove the TOTP secret
...
/*
AccountApiService Remove one IP range from the account allow-list (PATCH on /account/iplimits)
...
/*
AccountApiService Read full account profile, billing address, and security settings
...
/*
AccountApiService Fetch TOTP secret to enroll a 2FA authenticator app (Google Authenticator etc.)
...
/*
AccountApiService Aggregate dashboard payload — service counts, recent activity, alerts
...
/*
AccountApiService Global autocomplete across the caller&#x27;s services, domains, and records
...
/*
AccountApiService Destroy the current API/web session — token becomes unusable
...
/*
AccountApiService Sign out of the upstream OAuth provider session (does not unlink the account)
...
/*
AccountApiService Rotate the account&#x27;s REST/MCP API key — old key is invalidated immediately
...
/*
AccountApiService Toggle account-wide safety locks for password reset and OS reinstall
...
/*
AccountApiService Update contact and billing-address fields on the customer profile
...
/*
AccountApiService Add an IP CIDR/range to the account&#x27;s API+web allow-list (lockout-safe)
...
/*
AccountApiService Change the account login password (verifies current, kills other sessions)
...
/*
AccountApiService Set the account-level SSH public key auto-installed on new VPS/dedicated orders
...
/*
AccountApiService Verify TOTP code and enable two-factor authentication on the account
