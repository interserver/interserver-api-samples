...
val body : LicenseOrderRequest =  // LicenseOrderRequest | 
try {
    val result : ServiceOrderPostResponse = apiInstance.addLicense(body)
    println(result)
...
val id : kotlin.Int = 56 // kotlin.Int | The license service ID. Use `license_id` from `GET /licenses`.
try {
    val result : InlineResponse2005 = apiInstance.licensesCancel(id)
    println(result)
} catch (e: ClientException) {
