...
# **addFloatingIp**
> ServiceOrderPostResponse addFloatingIp(body)
...
let body = new InterServerManagementApi.FloatingIpOrderRequest(); // FloatingIpOrderRequest | 

apiInstance.addFloatingIp(body, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
...
# **floatingIpsCancel**
> InlineResponse2004 floatingIpsCancel(id)
...
apiInstance.putFloatingIps(body, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
