...
/* When true and enableDomainRegistering=true from the catalog, also registers/transfers the domain through the order. */
  ...
  @override
  String toString() {
    return 'WebsiteOrderPutRequest[hostname=$hostname, packageId=$packageId, rootpass=$rootpass, period=$period, coupon=$coupon, serviceOfferId=$serviceOfferId, script=$script, comment=$comment, registerDomain=$registerDomain, ]';
  }

  WebsiteOrderPutRequest.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
  ...
  static List<WebsiteOrderPutRequest> listFromJson(List<dynamic> json) {
    return json == null ? new List<WebsiteOrderPutRequest>() : json.map((value) => new WebsiteOrderPutRequest.fromJson(value)).toList();
  }

  static Map<String, WebsiteOrderPutRequest> mapFromJson(Map<String, Map<String, dynamic>> json) {
    var map = new Map<String, WebsiteOrderPutRequest>();
    if (json != null && json.length > 0) {
      json.forEach((String key, Map<String, dynamic> value) => map[key] = new WebsiteOrderPutRequest.fromJson(value));
    }
    return map;
  }
}
