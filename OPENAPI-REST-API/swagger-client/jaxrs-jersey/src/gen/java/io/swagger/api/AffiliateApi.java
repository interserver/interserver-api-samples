...
@SecurityRequirement(name = "sessionIdHeaderAuth")    }, tags={ "Billing" })
    @ApiResponses(value = { 
        ...
        @ApiResponse(responseCode = "401", description = "Unauthorized", content = @Content(mediaType = "application/json", schema = @Schema(implementation = InlineResponse401.class))) })
    public Response getAffiliateDownload(@Parameter(in = ParameterIn.QUERY, description = "Filter by status.") @QueryParam("st") String st,@Parameter(in = ParameterIn.QUERY, description = "Export format: csv, xls, xlsx, or pdf. Defaults to csv.", schema=@Schema(allowableValues={ "csv", "xls", "xlsx", "pdf" })
) @QueryParam("ex") String ex,@Parameter(in = ParameterIn.QUERY, description = "Year to filter the report. Defaults to the current year.") @QueryParam("year") Integer year,@Context SecurityContext securityContext)
    throws NotFoundException {
        return delegate.getAffiliateDownload(st,ex,year,securityContext);
    }
    @GET
    @Path("/rich_report")
...
@SecurityRequirement(name = "sessionIdHeaderAuth")    }, tags={ "Billing" })
    @ApiResponses(value = { 
        ...
        @ApiResponse(responseCode = "401", description = "Unauthorized", content = @Content(mediaType = "application/json", schema = @Schema(implementation = InlineResponse401.class))) })
    public Response getAffiliateSignups(@Parameter(in = ParameterIn.QUERY, description = "Filter signups by status. Use `default` to show all or pass a specific status value to narrow results.") @QueryParam("st") String st,@Context SecurityContext securityContext)
    throws NotFoundException {
        return delegate.getAffiliateSignups(st,securityContext);
    }
    @GET
    @Path("/traffic_graph")
