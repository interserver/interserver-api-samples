//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:interserver_api/api.dart';
import 'package:test/test.dart';

// tests for Server
void main() {
  // final instance = Server();

  group('test Server', () {
    // bool ipmiAuth
    test('to test the property `ipmiAuth`', () async {
      // TODO
    });

    // List<ServerClientLink> clientLinks (default value: const [])
    test('to test the property `clientLinks`', () async {
      // TODO
    });

    // ServerBillingDetails billingDetails
    test('to test the property `billingDetails`', () async {
      // TODO
    });

    // String custCurrency
    test('to test the property `custCurrency`', () async {
      // TODO
    });

    // String custCurrencySymbol
    test('to test the property `custCurrencySymbol`', () async {
      // TODO
    });

    // String package
    test('to test the property `package`', () async {
      // TODO
    });

    // List<String> serviceExtra (default value: const [])
    test('to test the property `serviceExtra`', () async {
      // TODO
    });

    // ServerLocations locations
    test('to test the property `locations`', () async {
      // TODO
    });

    // ServerNetworkInfo networkInfo
    test('to test the property `networkInfo`', () async {
      // TODO
    });

    // ServerExtraInfoTables extraInfoTables
    test('to test the property `extraInfoTables`', () async {
      // TODO
    });

    // ServerServiceInfo serviceInfo
    test('to test the property `serviceInfo`', () async {
      // TODO
    });


  });

}
