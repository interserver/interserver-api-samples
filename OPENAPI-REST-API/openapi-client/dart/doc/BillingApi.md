...
final api_instance = BillingApi();
final st = st_example; // String | Filter by status.
final ex = ex_example; // String | Export format: csv, xls, xlsx, or pdf. Defaults to csv.
final year = 56; // int | Year to filter the report. Defaults to the current year.

try {
    api_instance.getAffiliateDownload(st, ex, year);
} catch (e) {
...
final api_instance = BillingApi();
final st = st_example; // String | Filter signups by status. Use `default` to show all or pass a specific status value to narrow results.

try {
    final result = api_instance.getAffiliateSignups(st);
    print(result);
} catch (e) {
...
try {
    final result = api_instance.patchBillingCreditCardVerify(id, patchBillingCreditCardVerifyRequest);
    print(result);
} catch (e) {
