...
try {
    final result = api_instance.addServer(serverOrderPostRequest);
    print(result);
} catch (e) {
...
final api_instance = ServersApi();
final ids = 2313,2314,2315; // String | Comma-separated list of Server IDs to query (e.g. `2313,2314,2315`). May also be passed as repeated `ids[]` query parameters.

try {
    final result = api_instance.serverBulkIpmiPowerGet(ids);
    print(result);
} catch (e) {
