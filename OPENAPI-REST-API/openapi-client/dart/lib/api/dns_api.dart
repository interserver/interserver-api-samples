...
class DNSApi {
  ...
  Future<Response> addDnsDomainWithHttpInfo(String domain, String ip, { Future<void>? abortTrigger, }) async {
  ...
  /// * [String] ip (required):
  ///   IP Address to point the domain to.
  Future<SuccessTextResponse?> addDnsDomain(String domain, String ip, { Future<void>? abortTrigger, }) async {
    final response = await addDnsDomainWithHttpInfo(domain, ip, abortTrigger: abortTrigger,);
  ...
  /// * [int] prio:
  ///   Priority
  Future<Response> addDnsRecordWithHttpInfo(String id, String name, DnsRecordType type, String content, { int? ttl, int? prio, Future<void>? abortTrigger, }) async {
    // ignore: prefer_const_declarations
    final path = r'/dns/{id}'
      .replaceAll('{id}', id);
  ...
  /// * [int] prio:
  ///   Priority
  Future<void> addDnsRecord(String id, String name, DnsRecordType type, String content, { int? ttl, int? prio, Future<void>? abortTrigger, }) async {
    final response = await addDnsRecordWithHttpInfo(id, name, type, content, ttl: ttl, prio: prio, abortTrigger: abortTrigger,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
  }
  ...
  Future<Response> deleteDnsDomainWithHttpInfo(String id, { Future<void>? abortTrigger, }) async {
    // ignore: prefer_const_declarations
    final path = r'/dns/{id}'
      .replaceAll('{id}', id);
  ...
  /// * [String] id (required):
  ///   The DNS domain ID to delete. Use the `id` from `GET /dns` to identify the domain.
  Future<SuccessTextResponse?> deleteDnsDomain(String id, { Future<void>? abortTrigger, }) async {
    final response = await deleteDnsDomainWithHttpInfo(id, abortTrigger: abortTrigger,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  Future<Response> deleteDnsRecordWithHttpInfo(int domainId, int recordId, { Future<void>? abortTrigger, }) async {
    // ignore: prefer_const_declarations
  ...
  /// * [int] recordId (required):
  ///   The DNS record ID within the domain. Use the record `id` from `GET /dns/{id}` to identify the record.
  Future<SuccessTextResponse?> deleteDnsRecord(int domainId, int recordId, { Future<void>? abortTrigger, }) async {
    final response = await deleteDnsRecordWithHttpInfo(domainId, recordId, abortTrigger: abortTrigger,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  /// * [int] id (required):
  ///   The DNS domain ID. Use the `id` from `GET /dns` to identify the domain.
  Future<Response> getDnsDomainWithHttpInfo(int id, { Future<void>? abortTrigger, }) async {
    // ignore: prefer_const_declarations
    final path = r'/dns/{id}'
  ...
  /// * [int] id (required):
  ///   The DNS domain ID. Use the `id` from `GET /dns` to identify the domain.
  Future<List<DnsRecord>?> getDnsDomain(int id, { Future<void>? abortTrigger, }) async {
    final response = await getDnsDomainWithHttpInfo(id, abortTrigger: abortTrigger,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  ///
  /// Note: This method returns the HTTP [Response].
  Future<Response> getDnsListWithHttpInfo({ Future<void>? abortTrigger, }) async {
    // ignore: prefer_const_declarations
    final path = r'/dns';
  ...
  ///
  /// Returns every PowerDNS-hosted authoritative zone owned by the authenticated account, one row per zone, with the IP from the apex `A` record. Canonical entry point for discovering zone IDs before reading or editing records. The list is filtered server-side by session `account_id` — cross-account zones are never returned. Empty array means the account holds no zones (not an error). **Note:** this is the hosted DNS zone list, not registrar delegation — use the Domains tag's `updateDomainNameservers` to point a registered domain at `cdns1.interserver.net`/`cdns2.interserver.net`. Sibling ops: `getDnsDomain`, `addDnsDomain`, `addDnsRecord`, `deleteDnsDomain`.  **Path/Query/Body:** None.  **Returns:** Array of `DnsListItem`: - `id` (integer) — zone ID; pass to `getDnsDomain` / `addDnsRecord` / `deleteDnsDomain`. - `name` (string) — zone FQDN (e.g. `example.com`). - `content` (string) — IP from the apex `A` record matching the zone name (empty when no apex A exists yet).  **Auth:** Session/API key.  **Errors:** - `401` — unauthenticated.  **Related calls:** - **Per-zone record list:** `getDnsDomain`. - **Add a zone:** `addDnsDomain`. - **Add a record to an existing zone:** `addDnsRecord`. - **Registrar delegation:** `getDomainNameservers` / `updateDomainNameservers` (Domains tag). 
  Future<List<DnsListItem>?> getDnsList({ Future<void>? abortTrigger, }) async {
    final response = await getDnsListWithHttpInfo(abortTrigger: abortTrigger,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  ///
  /// * [String] auth:
  Future<Response> updateDnsRecordWithHttpInfo(int domainId, int recordId, { String? name, DnsRecordType? type, String? content, String? ttl, String? prio, String? disabled, String? ordername, String? auth, Future<void>? abortTrigger, }) async {
    // ignore: prefer_const_declarations
    final path = r'/dns/{domainId}/{recordId}'
      .replaceAll('{domainId}', domainId.toString())
      .replaceAll('{recordId}', recordId.toString());
  ...
  ///
  /// * [String] auth:
  Future<SuccessTextResponse?> updateDnsRecord(int domainId, int recordId, { String? name, DnsRecordType? type, String? content, String? ttl, String? prio, String? disabled, String? ordername, String? auth, Future<void>? abortTrigger, }) async {
    final response = await updateDnsRecordWithHttpInfo(domainId, recordId, name: name, type: type, content: content, ttl: ttl, prio: prio, disabled: disabled, ordername: ordername, auth: auth, abortTrigger: abortTrigger,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
