...
Protected Class ServersApi
	#tag Method, Flags = &h0
		Sub AddServer(, serverOrderPostRequest As OpenAPIClient.Models.ServerOrderPostRequest)
		  // Operation addServer
		  ...
		  localVarHTTPSocket.SetRequestContent(Xoson.toJSON(serverOrderPostRequest), "application/json")
	...
	#tag Method, Flags = &h0
		Sub GetServerInvoices(, id As Integer)
		  ...
		  // - Use to retrieve the invoice history for a single dedicated server — e.g. before a cancel, refund, or to show outstanding balances. Path param: `id` (integer server_id from `getServerList`). No body. Inherits from `MyAdminApiBillingInvoicesList` with module=servers. Returns: `ChargeInvoiceRows` array — invoice rows with id, date, amount, status, currency, line items. Errors: 404 if `id` not owned by the caller; 401 unauth. Sibling ops: `getServerInfo` (current service state), `serversCancel` (cancel), `getBillingInvoice` (single invoice by invoice id), `getVpsInvoices`/`getDomainInvoices` for other modules, `getServersWelcomeEmail` to resend setup info.
		  // - defaultResponse: Nil
		  //
		  // - API Key:
		  //   - type: apiKey sessionid 
	...
	#tag Method, Flags = &h0
		Sub ServerBulkIpmiPowerGet(, ids As String)
		  ...
		  //
		  // Invokes ServersApiCallbackHandler.ServerBulkIpmiPowerGetCallback(ServerBulkIpmiPowerResponse) on completion. 
		  //
		  ...
		  localVarQueryParams = localVarQueryParams + EncodeURLComponent("ids") + "=" + EncodeURLComponent(ids)
		  

		  If me.ApiKeysessionIdCookieAuth = "" Then Raise New OpenAPIClient.OpenAPIClientException(kErrorCannotAuthenticate, "API key is unset. Please assign a value to `ServersApi.ApiKeysessionIdCookieAuth` before invoking `ServersApi.ServerBulkIpmiPowerGet()`.")
		  
		  
		  If me.ApiKeyapiKeyAuth = "" Then Raise New OpenAPIClient.OpenAPIClientException(kErrorCannotAuthenticate, "API key is unset. Please assign a value to `ServersApi.ApiKeyapiKeyAuth` before invoking `ServersApi.ServerBulkIpmiPowerGet()`.")
		  
		  localVarHTTPSocket.SetRequestHeader(EncodeURLComponent("X-API-KEY"), EncodeURLComponent(me.ApiKeyapiKeyAuth))
		  If me.ApiKeysessionIdHeaderAuth = "" Then Raise New OpenAPIClient.OpenAPIClientException(kErrorCannotAuthenticate, "API key is unset. Please assign a value to `ServersApi.ApiKeysessionIdHeaderAuth` before invoking `ServersApi.ServerBulkIpmiPowerGet()`.")
		  
		  localVarHTTPSocket.SetRequestHeader(EncodeURLComponent("sessionid"), EncodeURLComponent(me.ApiKeysessionIdHeaderAuth))
		  


		  Dim localVarPath As String = "/servers/bulk/ipmi_power"
		  
		  
		  
		  AddHandler localVarHTTPSocket.PageReceived, addressof me.ServerBulkIpmiPowerGet_handler
		  AddHandler localVarHTTPSocket.Error, addressof Me.ServerBulkIpmiPowerGet_error
	...
	#tag Method, Flags = &h21
		Private Function ServerBulkIpmiPowerGetPrivateFuncDeserializeResponse(HTTPStatus As Integer, Headers As InternetHeaders, error As OpenAPIClient.OpenAPIClientException, Content As String, ByRef outData As OpenAPIClient.Models.ServerBulkIpmiPowerResponse) As Boolean
		  Dim contentType As String = Headers.Value("Content-Type")
		  Dim contentEncoding As TextEncoding = OpenAPIClient.EncodingFromContentType(contentType)
		  Content = DefineEncoding(Content, contentEncoding)
		  ...
		  If HTTPStatus > 199 and HTTPStatus < 300 then
		    If contentType.LeftB(16) = "application/json" then
			  ...
			  outData = New OpenAPIClient.Models.ServerBulkIpmiPowerResponse
			  Try
		        Xoson.fromJSON(outData, Content.toText())
	...
	#tag Method, Flags = &h21
		Private Sub ServerBulkIpmiPowerGet_error(sender As HTTPSecureSocket, Code As Integer)
		  If sender <> nil Then sender.Close()

		  Dim error As New OpenAPIClient.OpenAPIClientException(Code)
		  Dim data As OpenAPIClient.Models.ServerBulkIpmiPowerResponse
		  CallbackHandler.ServerBulkIpmiPowerGetCallback(error, data)
		End Sub
	#tag EndMethod

	#tag Method, Flags = &h21
		Private Sub ServerBulkIpmiPowerGet_handler(sender As HTTPSecureSocket, URL As String, HTTPStatus As Integer, Headers As InternetHeaders, Content As String)
		  #Pragma Unused URL
		  ...
		  Dim data As OpenAPIClient.Models.ServerBulkIpmiPowerResponse
		  Call ServerBulkIpmiPowerGetPrivateFuncDeserializeResponse(HTTPStatus, Headers, error, Content, data)
