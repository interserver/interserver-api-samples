...
feature -- API Access
	...
	account_locales : detachable STRING_TABLE [GET_ACCOUNT_LOCALES_200_RESPONSE_VALUE]
			...
			-- 
			-- 
			-- Result STRING_TABLE [GET_ACCOUNT_LOCALES_200_RESPONSE_VALUE]
		require
		local
  			l_path: STRING
  			l_request: API_CLIENT_REQUEST
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { STRING_TABLE [GET_ACCOUNT_LOCALES_200_RESPONSE_VALUE] } l_response.data ({ STRING_TABLE [GET_ACCOUNT_LOCALES_200_RESPONSE_VALUE] }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
