...
feature -- API Access
	...
	add_server (server_order_post_request: SERVER_ORDER_POST_REQUEST): detachable ADD_SERVER_200_RESPONSE
			...
			-- 
			-- argument: server_order_post_request  (required)
			-- 
			-- 
		...
		do
			reset_error
			create l_request
			l_request.set_body(server_order_post_request)
	...
	server_bulk_ipmi_power_get (ids: STRING_32): detachable SERVER_BULK_IPMI_POWER_RESPONSE
			...
			-- 
			-- argument: ids Comma-separated list of Server IDs to query (e.g. &#x60;2313,2314,2315&#x60;). May also be passed as repeated &#x60;ids[]&#x60; query parameters. (required)
			-- 
			-- 
			-- Result SERVER_BULK_IPMI_POWER_RESPONSE
		require
		local
  			l_path: STRING
  			l_request: API_CLIENT_REQUEST
		...
		do
			...
			l_path := "/servers/bulk/ipmi_power"
			l_request.fill_query_params(api_client.parameter_to_tuple("", "ids", ids));
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { SERVER_BULK_IPMI_POWER_RESPONSE } l_response.data ({ SERVER_BULK_IPMI_POWER_RESPONSE }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
