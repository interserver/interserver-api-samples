...
-spec add_website(openapi_website_order_post_request:openapi_website_order_post_request()) ->
  openapi_utils:response().
add_website(OpenapiWebsiteOrderPostRequest) ->
  Method      = post,
  Host        = application:get_env(openapi, host, "http://localhost:8080"),
  Path        = ["/websites/order"],
  Body        = OpenapiWebsiteOrderPostRequest,
...
-spec put_websites(openapi_website_order_put_request:openapi_website_order_put_request()) ->
  openapi_utils:response().
put_websites(OpenapiWebsiteOrderPutRequest) ->
  Method      = put,
  Host        = application:get_env(openapi, host, "http://localhost:8080"),
  Path        = ["/websites/order"],
  Body        = OpenapiWebsiteOrderPutRequest,
  ContentType = hd(["application/json"]),
