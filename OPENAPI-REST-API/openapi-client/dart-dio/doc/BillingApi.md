...
final api = InterserverApi().getBillingApi();
...
final api = InterserverApi().getBillingApi();
final String st = st_example; // String | Filter by status.
final String ex = ex_example; // String | Export format: csv, xls, xlsx, or pdf. Defaults to csv.
final int year = 56; // int | Year to filter the report. Defaults to the current year.

try {
    api.getAffiliateDownload(st, ex, year);
} on DioException catch (e) {
    print('Exception when calling BillingApi->getAffiliateDownload: $e\n');
}
```
...
final api = InterserverApi().getBillingApi();
final String st = st_example; // String | Filter signups by status. Use `default` to show all or pass a specific status value to narrow results.

try {
    final response = api.getAffiliateSignups(st);
    print(response);
} on DioException catch (e) {
    print('Exception when calling BillingApi->getAffiliateSignups: $e\n');
}
```
...
> SuccessTextResponse patchBillingCreditCardVerify(patchBillingCreditCardVerifyRequest, id)
...
final api = InterserverApi().getBillingApi();
final PatchBillingCreditCardVerifyRequest patchBillingCreditCardVerifyRequest = ; // PatchBillingCreditCardVerifyRequest | 
final int id = 56; // int | The credit card ID to verify. Use the ID returned from `POST /billing/creditcards`.

try {
    final response = api.patchBillingCreditCardVerify(patchBillingCreditCardVerifyRequest, id);
    print(response);
} on DioException catch (e) {
    print('Exception when calling BillingApi->patchBillingCreditCardVerify: $e\n');
}
```
...
> SuccessTextResponse postBillingCreditCardVerify(billingVerifyCcRequest, id)
...
try {
    final response = api.postBillingCreditCardVerify(billingVerifyCcRequest, id);
    print(response);
} on DioException catch (e) {
