...
final api = InterserverApi().getFloatingIPsApi();
final FloatingIpOrderRequest floatingIpOrderRequest = ; // FloatingIpOrderRequest | 

try {
    final response = api.addFloatingIp(floatingIpOrderRequest);
    print(response);
...
try {
    final response = api.postFloatingIpsChangeIp(ip, id);
    print(response);
} on DioException catch (e) {
