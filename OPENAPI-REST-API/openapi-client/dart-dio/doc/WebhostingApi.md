# interserver_api.api.WebhostingApi
...
final api = InterserverApi().getWebhostingApi();
final WebsiteOrderPostRequest websiteOrderPostRequest = ; // WebsiteOrderPostRequest | 

try {
    final response = api.addWebsite(websiteOrderPostRequest);
    print(response);
...
# **postWebsiteBuyIp**
> PostWebsiteBuyIp200Response postWebsiteBuyIp(postWebsiteBuyIpRequest, id)
...
try {
    final response = api.postWebsiteBuyIp(postWebsiteBuyIpRequest, id);
    print(response);
} on DioException catch (e) {
...
> PostWebsiteMigration200Response postWebsiteMigration(postWebsiteMigrationRequest, id)
...
try {
    final response = api.postWebsiteMigration(postWebsiteMigrationRequest, id);
    print(response);
} on DioException catch (e) {
...
# **postWebsitesReverseDns**
> TextResponse postWebsitesReverseDns(reverseDnsEntries, id)
...
try {
    final response = api.postWebsitesReverseDns(reverseDnsEntries, id);
    print(response);
} on DioException catch (e) {
...
try {
    api.putWebsites(websiteOrderPutRequest);
} on DioException catch (e) {
