...
module InterserverApiClient
  class Configuration
    ...
    #
    # ```
    # config = InterserverApiClient::Configuration.new do |config|
    #   config.username = "xxx"
    #   config.password = "xxx"
    # end
    # ```
    ...
    def apply_auth!(headers : HTTP::Headers, params : Hash(String, String | Array(String)), auth_names : Array(String)) : Nil
      if auth_names.includes?("apiKeyAuth")
        value = api_key_with_prefix(:"X-API-KEY")
        unless value.empty?
          headers["X-API-KEY"] = value
        end
      end
      if auth_names.includes?("sessionIdHeaderAuth")
        value = api_key_with_prefix(:"sessionid")
        unless value.empty?
          headers["sessionid"] = value
        end
      end
      if auth_names.includes?("sessionIdCookieAuth")
        value = api_key_with_prefix(:"sessionid")
        unless value.empty?
