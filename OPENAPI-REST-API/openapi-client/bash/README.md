...
$ chmod u+x interserver-api

# Print the list of operations available on the service
$ ./interserver-api -h

# Print the service description
$ ./interserver-api --about

# Print detailed information about specific operation
$ ./interserver-api <operationId> -h

# Make GET request
./interserver-api --host http://<hostname>:<port> --accept xml <operationId> <queryParam1>=<value1> <header_key1>:<header_value2>

# Make GET request using arbitrary curl options (must be passed before <operationId>) to an SSL service using username:password
interserver-api -k -sS --tlsv1.2 --host https://<hostname> -u <user>:<password> --accept xml <operationId> <queryParam1>=<value1> <header_key1>:<header_value2>

# Make POST request
$ echo '<body_content>' | interserver-api --host <hostname> --content-type json <operationId> -
...
#   "key3": 23
# }
$ echo '<body_content>' | interserver-api --host <hostname> --content-type json <operationId> key1==value1 key2=value2 key3:=23 -

# Make POST request with form data
$ interserver-api --host <hostname> <operationId> key1:=value1 key2:=value2 key3:=23

# Preview the cURL command without actually executing it
$ interserver-api --host http://<hostname>:<port> --dry-run <operationid>
...
```shell
source interserver-api.bash-completion
```

Alternatively, the script can be copied to the `/etc/bash-completion.d` (or on OSX with Homebrew to `/usr/local/etc/bash-completion.d`):

```shell
sudo cp interserver-api.bash-completion /etc/bash-completion.d/interserver-api
```
...
In Zsh, the generated `_interserver-api` Zsh completion file must be copied to one of the folders under `$FPATH` variable.
