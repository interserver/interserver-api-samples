...
Step 3 of the Mail Baby order flow. Revalidates via 'validate_buy_mail()', then calls 'place_buy_mail()' to create a 'Repeat_Invoice' recurring billing row, an initial 'invoices' row, and a 'mail' service record in pending status. SMTP credentials become active once the activation worker runs the welcome email (after the invoice is paid).$(tput bold) Real money $(tput sgr0)— call 'putMail' first. Sibling ops: 'getNewMail', 'putMail', 'getMailInfo', 'initiatePayment'.

**Body fields:**
- 'serviceType' (integer, required) — plan id from 'getNewMail'.
- 'coupon' (string, optional).
- 'comment' (string, optional) — saved on the order row.
$(tput bold) Returns $(tput sgr0)(on success): '{continue: true, total_cost, iid, iids, real_iids, serviceId (new mail_id), invoice_description, cj_params}' — pass 'real_iids' to 'initiatePayment'. On validation failure: '{continue: false, errors: [...]}' with HTTP 200.
...
```bash
interserver-api createMailAlert '{
  "type" : "type",
  "value" : "value",
  "to" : "to",
  "enabled" : "enabled"
...
Hard-deletes a single alert row. Handler verifies the alert belongs to this service+module before deleting.$(tput bold) Irreversible $(tput sgr0)— no history is preserved; recreate via 'createMailAlert' if needed. Sibling ops: 'getMailAlerts', 'createMailAlert', 'updateMailAlert'.
...
```bash
interserver-api deleteMailAlert id=value
```
...
Hard-deletes a single 'mail_spam' row scoped to this service's 'mail_username'.$(tput bold) Irreversible $(tput sgr0)— no audit copy preserved. Query filter 'id={rule} AND user='{mail_username}'' prevents cross-tenant deletes; passing a 'rule' belonging to a different mail order is silently a no-op (still returns success). Sibling ops: 'getRules', 'addRule', 'updateRule'.
...
```bash
interserver-api deleteRule id=value rule=value
```
...
```bash
interserver-api delistBlock id=value
```
...
**Path param:**
- 'id' (integer, required) — 'mail_id' from 'getMailList'.
$(tput bold) Returns $(tput sgr0)(schema 'MailAlertsResponse'): array of alert rows.
...
```bash
interserver-api getMailAlerts id=value
```
...
**Path param:**
- 'id' (integer, required) — 'mail_id' from 'getMailList'.
$(tput bold) Returns $(tput sgr0)(schema 'MailBlocks'):
...
```bash
interserver-api getMailBlocks id=value
```
...
**Path param:**
- 'id' (integer, required) — 'mail_id' from 'getMailList'.
$(tput bold) Returns $(tput sgr0)(schema 'MailDelistResponse'):
...
```bash
interserver-api getMailDelist id=value
```
...
$(tput bold) Returns $(tput sgr0)(schema 'MailDeliverabilityResponse'):
...
```bash
interserver-api getMailDeliverability id=value
```
...
**Path param:**
- 'id' (integer, required) — 'mail_id' from 'getMailList'.
$(tput bold) Returns $(tput sgr0)(schema 'MailSchema'):
...
```bash
interserver-api getMailInfo id=value
```
...
```bash
interserver-api getMailInvoices id=value
```
...
```bash
interserver-api getMailWelcomeEmail id=value
```
...
**Path/Query/Body:** None.
$(tput bold) Returns $(tput sgr0)(schema 'MailOrder'):
...
```bash
interserver-api getRules id=value
```
...
$(tput bold) Returns $(tput sgr0)(schema 'MailStatsType'):
...
```bash
interserver-api getStats id=value  time=value
```
...
Cancels the Mail Baby service through the shared 'Billing\\CancelService::go($id)' flow with 'module='mail''. SMTP credentials are deactivated, the service transitions to canceled, the 'repeat_invoice' is stopped, and queued submissions stop being accepted.$(tput bold) Irreversible via API $(tput sgr0)— re-activation requires placing a new order via 'addMail'. Sibling ops: 'getMailInfo', 'getMailInvoices', 'addMail'.
...
```bash
interserver-api mailCancel id=value
```
...
```bash
interserver-api postMailDelist '{
  "unblock" : "unblock"
}' id=value
```
...
Generates a new 20-char SMTP password (lower/upper/digits via 'generate_password'), writes it to the ZoneMTA Mongo 'users' collection for username 'mb{mail_id}', logs the change to 'App::history()', and emails the result to the account-on-file via 'client_email.tpl'. **Any MTA, app, or saved client still using the old password will start failing auth immediately.** The new password is$(tput bold) not $(tput sgr0)returned in the response — fetch via 'getMailWelcomeEmail' or 'getMailInfo'. Sibling ops: 'getMailWelcomeEmail', 'getMailInfo'.
...
```bash
interserver-api resetMailPassword id=value
```
...
```bash
interserver-api updateMailAlert '{
  "alert_id" : 0,
  "type" : "type",
  "value" : "value",
  "to" : "to",
...
```bash
interserver-api updateMailInfo id=value
```
 ...
 **id** | **integer** | The mail service ID. Use 'mail_id' from 'GET /mail'. | [default to null]
 **rule** | **string** | The ID of the deny rule to update. | [default to null]
...
$(tput bold) Returns $(tput sgr0)(schema 'MailLog'):
...
```bash
interserver-api viewMailLog id=value  id=value  origin=value  mx=value  from=value  to=value  subject=value  mailid=value  messageId=value  replyto=value  headerfrom=value  delivered=value  skip=value  limit=value  startDate=value  endDate=value  sort=value  dir=value  groupby=value
```
 ...
 **startDate** | [**ViewMailLogStartDateParameter**](.md) | Earliest date to include.  Accepts either a Unix timestamp (integer seconds since epoch) or a date string parseable by 'strtotime()' such as '2024-01-15' or 'last monday'.  Messages with a 'time' value$(tput bold) greater than or equal to $(tput sgr0)this value will be included. | [optional] [default to null]
 **endDate** | [**ViewMailLogStartDateParameter**](.md) | Latest date to include.  Accepts either a Unix timestamp (integer seconds since epoch) or a date string parseable by 'strtotime()' such as '2024-01-31' or 'yesterday'. Messages with a 'time' value$(tput bold) less than or equal to $(tput sgr0)this value will be included. | [optional] [default to null]
