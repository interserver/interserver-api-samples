...
```bash
interserver-api addBillingCreditCard '{
  "name" : "John Doe",
...
```bash
interserver-api addBillingPrepay '{
  "module" : "module",
...
Removes the indexed credit card from the account's 'ccs' collection. If the deleted card was also the account's primary 'cc', the primary field is cleared — 'initiatePayment' ('method=cc') will then return an error until a new default is designated via 'updateBillingPaymentMethod'.$(tput bold) Irreversible $(tput sgr0)— to re-store the same card, re-run 'addBillingCreditCard'. Sibling ops: 'addBillingCreditCard', 'updateBillingCreditCard', 'updateBillingPaymentMethod', 'getBillingCreditCardVerify'.
...
```bash
interserver-api deleteBillingCreditCard id=value
```
...
-$(tput bold) Prepay invoice $(tput sgr0)(description matches 'Prepay ID N Invoice') — delegates to 'deleteBillingPrepay($pid)'.
-$(tput bold) Initial service invoice $(tput sgr0)('repeat_invoices_id == service._invoice') — deletes:
...
```bash
interserver-api deleteBillingInvoice id=value
```
...
Removes a prepay from the account, with one safety rule: a prepay that still has usable credit ('prepay_remaining > $0.01') cannot be deleted$(tput dim) unless $(tput sgr0)it also has unpaid funding invoices we can clean up — in which case those unpaid 'invoices' rows are deleted and the prepay row stays. Use to back out a never-funded prepay, or to surface stuck unpaid funding invoices.$(tput bold) Irreversible $(tput sgr0)— funded credit is unrecoverable through this endpoint. Sibling ops: 'getBillingPrePays', 'addBillingPrepay', 'deleteBillingInvoice'.
...
```bash
interserver-api deleteBillingPrepay id=value
```
...
```bash
interserver-api getAffiliateBanners
```
...
```bash
interserver-api getAffiliateDownload  st=value  ex=value  year=value
```
...
Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **st** | **string** | Filter by status. | [optional] [default to null]
 **ex** | **string** | Export format: csv, xls, xlsx, or pdf. Defaults to csv. | [optional] [default to null]
 **year** | **integer** | Year to filter the report. Defaults to the current year. | [optional] [default to null]
...
Returns a server-rendered HTML/text summary report combining commission totals, conversion rates, and traffic in one round-trip — useful for embedding in a dashboard panel. The payload is$(tput bold) not structured JSON $(tput sgr0)— for chart-friendly data use 'getAffiliateSalesGraph' and 'getAffiliateTrafficGraph' instead. Backed by 'affiliate_summary_report()'. Sibling ops: 'getAffiliateSalesGraph', 'getAffiliateTrafficGraph', 'getAffiliateSignups', 'getAffiliateDownload', 'getAffiliateWebTraffic'.
...
```bash
interserver-api getAffiliateRichReport
```
...
```bash
interserver-api getAffiliateSalesGraph  days=value
```
...
```bash
interserver-api getAffiliateSignups  st=value
```
...
Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **st** | **string** | Filter signups by status. Use 'default' to show all or pass a specific status value to narrow results. | [optional] [default to null]
...
```bash
interserver-api getAffiliateTrafficGraph  days=value
```
...
```bash
interserver-api getAffiliateWebTraffic
```
...
```bash
interserver-api getBillingCart
```
...
```bash
interserver-api getBillingCreditCardVerify id=value
```
...
```bash
interserver-api getBillingInvoice id=value
```
...
```bash
interserver-api getBillingInvoices
```
...
```bash
interserver-api getBillingPrePays
```
...
$(tput bold) Returns $(tput sgr0)(one of three shapes — branch on 'type'):
...
$(tput bold)$(tput setaf 7)1) Order created — POST /vps/order returned {serviceid: 12345, real_iids: [\"25296600\"]}$(tput sgr0)

$(tput bold)$(tput setaf 7)2) Pay with stored credit card:$(tput sgr0)
GET /apiv2/billing/pay/cc/25296600

$(tput bold)$(tput setaf 7)3) Response:$(tput sgr0)
{\"type\": \"single\", \"text\": \"Payment processed.\"}

$(tput bold)$(tput setaf 7)4) Poll service:$(tput sgr0)
...
$(tput bold)$(tput setaf 7)Client redirects user; PayPal IPN later marks invoice paid and activates service.$(tput sgr0)
---

### Example

```bash
interserver-api initiatePayment method=value invoices=value
```
...
```bash
interserver-api patchBillingCreditCardVerify id=value
```
...
```bash
interserver-api postBillingCreditCardVerify '{
  "idx" : 0,
  "cc_ccv2" : "cc_ccv2",
...
```bash
interserver-api updateAffiliateDockSetup
```
...
```bash
interserver-api updateAffiliatePaymentSetup
```
...
```bash
interserver-api updateBillingCreditCard id=value
```
...
```bash
interserver-api updateBillingPaymentMethod '{
