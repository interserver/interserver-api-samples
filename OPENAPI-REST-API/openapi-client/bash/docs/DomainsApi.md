...
$(tput bold) Returns $(tput sgr0)(schema 'ServiceOrderPostResponse'): '{total_cost, iid, iids, real_iids, serviceid (new domain_id), invoice_description, cj_params, payUrl}' — pass 'real_iids' to 'initiatePayment' to fund the order.
...
```bash
interserver-api addDomainDnssec '{
  "algorithm" : [ 0, 0 ],
  "digest_type" : [ 6, 6 ],
...
```bash
interserver-api addDomainNameserver '{
  "name" : "ns1.host.com",
...
- Does$(tput bold) not $(tput sgr0)call the registrar — the registration remains active at OpenSRS until natural expiry.
...
```bash
interserver-api cancelDomain id=value
```
...
```bash
interserver-api deleteDomainDnssec id=value
```
...
```bash
interserver-api deleteDomainNameserver id=value  index=value
```
...
**Path param:**
- 'id' (integer, required) — 'domain_id' from 'getDomainsList'.
$(tput bold) Returns $(tput sgr0)(schema 'DomainContactDetails'):
...
```bash
interserver-api getDomainContact id=value
```
...
**Path param:**
- 'id' (integer, required) — 'domain_id' from 'getDomainsList'.
$(tput bold) Returns $(tput sgr0)(schema 'DomainDnssecRecords'):
...
```bash
interserver-api getDomainDnssec id=value
```
...
**Path param:**
- 'id' (integer, required) — 'domain_id' from 'getDomainsList'.
$(tput bold) Returns $(tput sgr0)(schema 'Domain'):
...
```bash
interserver-api getDomainInfo id=value
```
...
```bash
interserver-api getDomainInvoices id=value
```
...
```bash
interserver-api getDomainLookup name=value
```
...
Returns the registered-nameserver / glue-record entries currently set for the domain at OpenSRS. **Important distinction:** these are$(tput dim) registered nameservers $(tput sgr0)('host.example.com' + glue IP records the registrant has created at the registry), NOT the domain's delegation NS records. To replace the delegation set, use 'updateDomainNameservers'. Empty array means no glue records are registered. Sibling ops: 'addDomainNameserver', 'updateDomainNameservers', 'deleteDomainNameserver'.

**Path param:**
- 'id' (integer, required) — 'domain_id' from 'getDomainsList'.
$(tput bold) Returns $(tput sgr0)(schema 'DomainNameserverGetResponse'):
...
```bash
interserver-api getDomainNameservers id=value
```
...
```bash
interserver-api getDomainRenewal id=value
```
...
$(tput bold) Returns $(tput sgr0)(schema 'DomainSearchResponse'):
...
```bash
interserver-api getDomainSearch name=value
```
...
- When a transfer: '{transfer: true, info: {status, statusText, type, transferrable, reason}}' where 'status' is one of:
  ...
  - 'pending_owner' —$(tput bold) customer action required $(tput sgr0)(approve the OpenSRS email).
...
```bash
interserver-api getDomainTransfer id=value
```
...
```bash
interserver-api getDomainWhoisPrivacy id=value
```
...
```bash
interserver-api getDomainsList
```
...
```bash
interserver-api getDomainsWelcomeEmail id=value
```
...
**Path/Query/Body:** None.
$(tput bold) Returns $(tput sgr0)(schema 'DomainOrder'):
...
```bash
interserver-api postDomainRenewal id=value
```
...
```bash
interserver-api postDomainSearch name=value
```
...
Re-polls OpenSRS transfer state. Behaves identically to 'getDomainTransfer' (same 'go()' handler) — provided so dashboards can refresh via a form-action pattern.$(tput bold) This endpoint does not initiate transfers $(tput sgr0)— to start a transfer, use 'addDomain' with 'type=transfer'. Sibling ops: 'getDomainTransfer', 'addDomain' (initiate), 'getDomainContact' (set 'auth_info').
...
```bash
interserver-api postDomainTransfer id=value
```
...
Pushes updated contact data to the registrar via OpenSRS 'provModify', applied to admin/tech/billing/owner contact roles via 'also_apply_to'. Active services apply changes immediately at the registrar; pending services may trigger 'queue_process_payment' when the order is paid.$(tput bold) Domain must not be locked $(tput sgr0)— locked domains return an error directing the user to unlock first. **Note:** registrant-name changes on some TLDs (e.g. '.com', '.net') require a 60-day transfer lock per ICANN rules. Sibling ops: 'getDomainContact', 'getDomainInfo'.
...
```bash
interserver-api updateDomainContact '{
  "status" : "status",
  "state" : "state",
  "org_name" : "org_name",
  "country" : "country",
...
Account-level write-back hook for the domain service record. Runs the same 'View::go()' handler as 'getDomainInfo' — it does NOT push registrar-side changes by itself.$(tput bold) For specific changes use the dedicated endpoints $(tput sgr0)— they push to OpenSRS where appropriate. Sibling ops: 'getDomainInfo', 'updateDomainContact', 'updateDomainNameservers', 'addDomainDnssec', 'updateDomainWhoisPrivacy'.
...
```bash
interserver-api updateDomainInfo id=value
```
...
Atomically replaces the domain's authoritative-nameserver delegation list via OpenSRS 'nsAdvancedUpdt' with 'op_type=assign'. **Domain must be unlocked.** Use this to switch DNS providers (e.g. point at InterServer 'cdns1'/'cdns2', Cloudflare, AWS Route53, etc.).$(tput bold) Replaces the full set in one call $(tput sgr0)— partial updates are not supported through this endpoint. To register a new glue record (host + IP) at the registry, use 'addDomainNameserver'. Sibling ops: 'getDomainNameservers', 'addDomainNameserver', 'deleteDomainNameserver', 'addDnsDomain' (host a zone at InterServer).
...
```bash
interserver-api updateDomainNameservers '{
  "nameserver" : [ "ns1.example.com", "ns1.example.com" ]
}' id=value
```
...
```bash
interserver-api updateDomainWhoisPrivacy '{
  "func" : "enable",
