...
$(tput bold) Returned fields $(tput sgr0)(schema 'ServiceOrderPostResponse'):
...
Removes a backup file from storage to free space. For 'minio'-typed backups runs 'mc rm --force --recursive' on the path; for 'swift'-typed backups removes the storage object via the Swift API. ZFS-typed backups$(tput bold) cannot $(tput sgr0)be deleted through this endpoint — they return an error directing the caller to open a support ticket.$(tput bold) Irreversible $(tput sgr0)— once deleted the backup cannot be used with 'postVpsRestore' or 'downloadVpsBackup'. Sibling ops: 'getVpsBackups' (list), 'downloadVpsBackup' (download first), 'getVpsBackup' (create new).
...
```bash
interserver-api deleteVpsBackup id=value  file=value  all=value
```
...
```bash
interserver-api doVpsBlockSmtp id=value
```
...
```bash
interserver-api doVpsDisableCd id=value
```
...
```bash
interserver-api doVpsDisableQuota id=value
```
...
```bash
interserver-api doVpsEjectCd id=value
```
...
```bash
interserver-api doVpsEnableQuota id=value
```
...
```bash
interserver-api doVpsRestart id=value
```
...
```bash
interserver-api doVpsStart id=value
```
...
```bash
interserver-api doVpsStop id=value
```
...
```bash
interserver-api downloadVpsBackup id=value  all=value
```
...
- HyperV, OpenVZ, SSD-OpenVZ, Virtuozzo, SSD-Virtuozzo:$(tput bold) disabled $(tput sgr0)server-side (returns 400 \"Backups are disabled for this type\").
...
```bash
interserver-api getVpsBackup id=value
```
...
```bash
interserver-api getVpsBackups id=value  all=value
```
...
```bash
interserver-api getVpsBuyHdSpace id=value
```
...
```bash
interserver-api getVpsBuyIp id=value
```
...
```bash
interserver-api getVpsChangeHostname id=value
```
...
```bash
interserver-api getVpsChangeRootPassword id=value
```
...
```bash
interserver-api getVpsChangeTimezone id=value
```
...
**Path param:**
- 'id' (integer, required) — VPS id from 'getVpsList.vps_id'.
$(tput bold) Returned fields $(tput sgr0)(schema 'Vps' plus extras):
...
```bash
interserver-api getVpsInfo id=value
```
...
```bash
interserver-api getVpsInsertCd id=value
```
...
```bash
interserver-api getVpsInvoices id=value
```
...
- 'templates' (array) — one entry per available template:
  ...
  - 'template_file' (string) —$(tput bold) canonical identifier $(tput sgr0)to pass to 'postVpsReinstallOs' (e.g. 'centos-7-x86_64.qcow2').
...
```bash
interserver-api getVpsReinstallOs id=value
```
...
```bash
interserver-api getVpsResetPassword id=value
```
...
```bash
interserver-api getVpsReverseDns id=value
```
...
```bash
interserver-api getVpsSetupVnc id=value
```
...
```bash
interserver-api getVpsSlices id=value
```
...
```bash
interserver-api getVpsTrafficUsage id=value
```
...
```bash
interserver-api getVpsViewDesktop id=value
```
...
```bash
interserver-api getVpsWelcomeEmail id=value
```
...
```bash
interserver-api postVpsBuyHdSpace id=value
```
...
```bash
interserver-api postVpsBuyIp id=value
```
...
```bash
interserver-api postVpsChangeHostname id=value
```
...
```bash
interserver-api postVpsChangeRootPassword id=value
```
...
**Body fields:**
- 'timezone' (string, required) — IANA identifier;$(tput bold) must $(tput sgr0)be one of the values returned by 'getVpsChangeTimezone' (in-array check enforced server-side).
...
```bash
interserver-api postVpsChangeTimezone id=value
```
...
```bash
interserver-api postVpsChangeWebuzoPassword id=value
```
...
```bash
interserver-api postVpsInsertCd id=value
```
...
```bash
interserver-api postVpsReinstallOs id=value
```
...
Resets the VPS root/Administrator password to a fresh random value generated on the hypervisor — typical when the customer has lost the existing password and cannot recover it. Queues a 'reset_password' action on the hypervisor ('vpsqueue'). The new password is delivered via the standard hypervisor password-change channel (logged to 'history_log' and surfaced through 'getVpsWelcomeEmail' / dashboard credential views).$(tput bold) No rollback $(tput sgr0)— to undo, set a new password via 'postVpsChangeRootPassword' or call 'postVpsResetPassword' again. Sibling ops: 'getVpsResetPassword', 'postVpsChangeRootPassword', 'postVpsChangeWebuzoPassword'.
...
```bash
interserver-api postVpsResetPassword id=value
```
...
```bash
interserver-api postVpsRestore '{
...
```bash
interserver-api postVpsReverseDns '{
  "ips" : {
...
```bash
interserver-api postVpsSetupVnc id=value
```
...
Changes the VPS's slice count — bumps or shrinks CPU/RAM/disk allocation. Deletes any unpaid prior slice-upgrade invoices for this VPS, updates the recurring 'repeat_invoices' row to the new slice count's cost, creates a one-off 'invoices' row prorated for the rest of the cycle, and either auto-opens a support ticket (active services — slice upgrades typically need a manual hypervisor action) or queues 'set_slices' directly. Downgrades complete free and immediately ('paid=1' zero-amount invoice).$(tput bold) Real money $(tput sgr0)on upgrades. Sibling ops: 'getVpsSlices', 'postVpsBuyHdSpace', 'initiatePayment', 'getVpsInfo'.
...
```bash
interserver-api postVpsSlices id=value
```
...
```bash
interserver-api postVpsTrafficUsage id=value
```
...
```bash
interserver-api postVpsViewDesktop id=value
```
...
$(tput bold) Returned fields $(tput sgr0)(schema 'VpsOrderPutResponse'):
...
```bash
interserver-api putVpsBuyHdSpace id=value
```
...
```bash
interserver-api updateVpsInfo id=value
```
...
```bash
interserver-api vPSCancel id=value
```
