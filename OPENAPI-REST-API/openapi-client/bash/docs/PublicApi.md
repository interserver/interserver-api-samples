...
```bash
interserver-api getAccountCurrencies
```
...
```bash
interserver-api getAccountLocales
```
...
```bash
interserver-api getCountries  fetch_by=value
```
...
**Path/Query/Body:** None.
$(tput bold) Returns $(tput sgr0)'{ logo, captcha, language, counts }':
...
```bash
interserver-api getOauthRedirect  provider=value
```
...
$(tput bold) Body fields $(tput sgr0)(JSON or form):
...
```bash
interserver-api patchOauthTwoFactor
```
...
```bash
interserver-api postOauthCallback  provider=value
```
