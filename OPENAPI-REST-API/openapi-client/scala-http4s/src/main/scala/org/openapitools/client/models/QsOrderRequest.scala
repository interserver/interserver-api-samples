...
object QsOrderRequest {
  given encoderQsOrderRequest: Encoder[QsOrderRequest] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
        Some("server" -> t.server.asJson),
  ...
  given decoderQsOrderRequest: Decoder[QsOrderRequest] = Decoder.instance { c =>
    for {
      server <- c.downField("server").as[Int]
      password <- c.downField("password").as[String]
      tos <- c.downField("tos").as[Boolean]
      os <- c.downField("os").as[Option[String]]
      comment <- c.downField("comment").as[Option[String]]
    } yield QsOrderRequest(
      server = server,
      password = password,
      tos = tos,
      os = os,
