...
object LicenseOrderRequest {
  given encoderLicenseOrderRequest: Encoder[LicenseOrderRequest] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderLicenseOrderRequest: Decoder[LicenseOrderRequest] = Decoder.instance { c =>
    for {
      `package` <- c.downField("package").as[Int]
