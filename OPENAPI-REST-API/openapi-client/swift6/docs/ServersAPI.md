...
# **addServer**
```swift
    open class func addServer(serverOrderPostRequest: ServerOrderPostRequest, completion: @escaping (_ data: AddServer200Response?, _ error: Error?) -> Void)
```
...
let serverOrderPostRequest = ServerOrderPostRequest(cpu: 123, hd: ServerOrderPostRequest_hd(), memory: 123, bandwidth: 123, ips: 123, os: 123, cp: 123, raid: 123, region: 123, servername: "servername_example", rootpass: "rootpass_example", tos: false, comment: "comment_example") // ServerOrderPostRequest | 

// Place a custom dedicated server order, creating a real billable invoice
ServersAPI.addServer(serverOrderPostRequest: serverOrderPostRequest) { (response, error) in
...
let _id = 987 // Int | Server ID number.

// Get full hardware, network, and lifecycle details for a dedicated server
ServersAPI.getServerInfo(_id: _id) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
// List billing invoices (charges + payments) tied to one dedicated server
ServersAPI.getServerInvoices(_id: _id) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
// List current reverse-DNS (PTR) records for a dedicated server's IPs
ServersAPI.getServerReverseDns(_id: _id) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
// Resend the dedicated server welcome email with setup credentials
ServersAPI.getServersWelcomeEmail(_id: _id) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
```swift
    open class func postServerReverseDns(_id: Int, reverseDnsEntries: ReverseDnsEntries, completion: @escaping (_ data: TextResponse?, _ error: Error?) -> Void)
```
...
// Update reverse-DNS (PTR) hostnames on a dedicated server's IPs
ServersAPI.postServerReverseDns(_id: _id, reverseDnsEntries: reverseDnsEntries) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
let ids = "ids_example" // String | Comma-separated list of Server IDs to query (e.g. `2313,2314,2315`). May also be passed as repeated `ids[]` query parameters.

// Read IPMI chassis power status for many dedicated servers in one call
ServersAPI.serverBulkIpmiPowerGet(ids: ids) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
// Read current IPMI Live whitelist + KVM gateway URL for a dedicated server
ServersAPI.serverIpmiLiveGet(_id: _id) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
# **serverIpmiLivePost**
```swift
    open class func serverIpmiLivePost(_id: Int, ip: String, asset: Int? = nil, completion: @escaping (_ data: ServerIpmiLiveInfo?, _ error: Error?) -> Void)
```
...
// Whitelist an IP for IPMI Live KVM gateway access (3-hour lease)
ServersAPI.serverIpmiLivePost(_id: _id, ip: ip, asset: asset) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
// Read IPMI chassis power status for a dedicated server (single)
ServersAPI.serverIpmiPowerGet(_id: _id) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
# **serverIpmiPowerPost**
```swift
    open class func serverIpmiPowerPost(_id: Int, action: Action_serverIpmiPowerPost, asset: Int? = nil, completion: @escaping (_ data: TextResponse?, _ error: Error?) -> Void)
```
...
// DESTRUCTIVE — change chassis power state on a bare-metal server
ServersAPI.serverIpmiPowerPost(_id: _id, action: action, asset: asset) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
# **serversCancel**
```swift
    open class func serversCancel(_id: Int, completion: @escaping (_ data: ServersCancel200Response?, _ error: Error?) -> Void)
```
...
// Cancel a dedicated server service at the end of the current billing cycle
ServersAPI.serversCancel(_id: _id) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
let _id = "_id_example" // String | Server ID number.

// Update settings on a dedicated server order (shares handler with view)
ServersAPI.updateServerInfo(_id: _id) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
