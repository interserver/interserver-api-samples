# interserver_api_client.ServersApi
...
with interserver_api_client.ApiClient(configuration) as api_client:
    ...
    api_instance = interserver_api_client.ServersApi(api_client)
    server_order_post_request = interserver_api_client.ServerOrderPostRequest() # ServerOrderPostRequest | 

    try:
        # Place a custom dedicated server order, creating a real billable invoice
        api_response = api_instance.add_server(server_order_post_request)
...
with interserver_api_client.ApiClient(configuration) as api_client:
    # Create an instance of the API class
    api_instance = interserver_api_client.ServersApi(api_client)
    place_buy_now_server_request = interserver_api_client.PlaceBuyNowServerRequest() # PlaceBuyNowServerRequest |  (optional)
...
with interserver_api_client.ApiClient(configuration) as api_client:
    # Create an instance of the API class
    api_instance = interserver_api_client.ServersApi(api_client)
    ids = '2313,2314,2315' # str | Comma-separated list of Server IDs to query (e.g. `2313,2314,2315`). May also be passed as repeated `ids[]` query parameters.

    try:
        # Read IPMI chassis power status for many dedicated servers in one call
        api_response = api_instance.server_bulk_ipmi_power_get(ids)
        print("The response of ServersApi->server_bulk_ipmi_power_get:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling ServersApi->server_bulk_ipmi_power_get: %s\n" % e)
```
...
Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **ids** | **str**| Comma-separated list of Server IDs to query (e.g. &#x60;2313,2314,2315&#x60;). May also be passed as repeated &#x60;ids[]&#x60; query parameters. | 
