...
try {
    val result : ServiceOrderPostResponse = apiInstance.addMail(mailOrderRequest)
    println(result)
...
try {
    val result : SuccessTextResponse = apiInstance.deleteMailAlert(id, deleteMailAlertRequest)
    println(result)
} catch (e: ClientException) {
...
try {
    val result : GenericResponse = apiInstance.updateRule(id, rule, denyRuleNew)
    println(result)
} catch (e: ClientException) {
