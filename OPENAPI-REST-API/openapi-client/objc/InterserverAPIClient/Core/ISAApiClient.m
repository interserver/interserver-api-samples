...
#import "ISAJSONRequestSerializer.h"
#import "ISAQueryParamCollection.h"
#import "ISADefaultConfiguration.h"

NSString *const ISAResponseObjectErrorKey = @"ISAResponseObject";

static NSString * const kISAContentDispositionKey = @"Content-Disposition";

static NSDictionary * ISA__headerFieldsForResponse(NSURLResponse *response) {
...
static NSString * ISA__fileNameForResponse(NSURLResponse *response) {
    NSDictionary * headers = ISA__headerFieldsForResponse(response);
    if(!headers[kISAContentDispositionKey]) {
    ...
    NSString *contentDispositionHeader = headers[kISAContentDispositionKey];
...
@interface ISAApiClient ()

@property (nonatomic, strong, readwrite) id<ISAConfiguration> configuration;

@property (nonatomic, strong) NSArray<NSString*>* downloadTaskResponseTypes;

@end

@implementation ISAApiClient

#pragma mark - Singleton Methods

+ (instancetype) sharedClient {
    static ISAApiClient *sharedClient = nil;
...
- (instancetype)init {
    return [self initWithConfiguration:[ISADefaultConfiguration sharedConfig]];
}

- (instancetype)initWithBaseURL:(NSURL *)url {
    return [self initWithBaseURL:url configuration:[ISADefaultConfiguration sharedConfig]];
}
...
- (instancetype)initWithBaseURL:(NSURL *)url configuration:(id<ISAConfiguration>)configuration {
    ...
    if (self) {
        ...
        _timeoutInterval = 60;
        _responseDeserializer = [[ISAResponseDeserializer alloc] init];
        _sanitizer = [[ISASanitizer alloc] init];

        _downloadTaskResponseTypes = @[@"NSURL*", @"NSURL"];

        AFHTTPRequestSerializer* afhttpRequestSerializer = [AFHTTPRequestSerializer serializer];
        ISAJSONRequestSerializer * swgjsonRequestSerializer = [ISAJSONRequestSerializer serializer];
        _requestSerializerForContentType = @{kISAApplicationJSONType : swgjsonRequestSerializer,
...
- (NSURLSessionDataTask*) taskWithCompletionBlock: (NSURLRequest *)request completionBlock: (void (^)(id, NSError *))completionBlock {
    ...
    NSURLSessionDataTask *task = [self dataTaskWithRequest:request uploadProgress:nil downloadProgress:nil completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
        ISADebugLogResponse(response, responseObject,request,error);
        if(!error) {
        ...
        if (responseObject) {
            ...
            userInfo[ISAResponseObjectErrorKey] = responseObject;
        }
...
- (NSURLSessionDataTask*) downloadTaskWithCompletionBlock: (NSURLRequest *)request completionBlock: (void (^)(id, NSError *))completionBlock {
    ...
    NSURLSessionDataTask* task = [self dataTaskWithRequest:request uploadProgress:nil downloadProgress:nil completionHandler:^(NSURLResponse *response, id responseObject, NSError *error) {
        ...
        NSString *filename = ISA__fileNameForResponse(response);
...
- (NSURLSessionTask*) requestWithPath: (NSString*) path
    ...
    __weak id<ISASanitizer> sanitizer = self.sanitizer;
    ...
    [pathParams enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        ...
        safeString = ISAPercentEscapedStringFromString(safeString);
...
- (NSString*) pathWithQueryParamsToString:(NSString*) path queryParams:(NSDictionary*) queryParams {
    ...
    for(NSString * key in [queryParams keyEnumerator]){
        ...
        NSString *safeKey = ISAPercentEscapedStringFromString(key);
        if ([queryParam isKindOfClass:[NSString class]]){
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, ISAPercentEscapedStringFromString(queryParam)]];

        } else if ([queryParam isKindOfClass:[ISAQueryParamCollection class]]){
            ISAQueryParamCollection * coll = (ISAQueryParamCollection*) queryParam;
            ...
            if([format isEqualToString:@"multi"]) {
                for(id obj in values) {
                    ...
                    NSString * safeValue = ISAPercentEscapedStringFromString([NSString stringWithFormat:@"%@",obj]);
            ...
            NSString * safeValue = ISAPercentEscapedStringFromString([values componentsJoinedByString:separatorStyle]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        } else {
            NSString * safeValue = ISAPercentEscapedStringFromString([NSString stringWithFormat:@"%@",queryParam]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        }
        counter += 1;
    }
...
- (void) updateHeaderParams:(NSDictionary * *)headers queryParams:(NSDictionary * *)queries WithAuthSettings:(NSArray *)authSettings {
    ...
    id<ISAConfiguration> config = self.configuration;
