#' Create a new MailOrderRequest
#'
#' @description
#' Request body to validate (putMail) or place (addMail) a mail order. Service type ids come from getNewMail.serviceTypes.
#'
#' @docType class
#' @title MailOrderRequest
#' @description MailOrderRequest Class
#' @format An \code{R6Class} generator object
#' @field serviceType Mail plan service type id from getNewMail.serviceTypes (must be buyable). integer
#' @field coupon Coupon code. character [optional]
#' @field comment Free-form note saved on the service row (used on addMail). character [optional]
...
MailOrderRequest <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new MailOrderRequest class.
    #'
    ...
    #' @param comment Free-form note saved on the service row (used on addMail).. Default to "".
    ...
    #'
    #' @return MailOrderRequest as a base R list.
    #' @examples
    #' # convert array of MailOrderRequest (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert MailOrderRequest to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      MailOrderRequestObject <- list()
      if (!is.null(self$`serviceType`)) {
        MailOrderRequestObject[["serviceType"]] <-
          self$`serviceType`
      }
      if (!is.null(self$`coupon`)) {
        MailOrderRequestObject[["coupon"]] <-
          self$`coupon`
      }
      if (!is.null(self$`comment`)) {
        MailOrderRequestObject[["comment"]] <-
          self$`comment`
      }
      return(MailOrderRequestObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of MailOrderRequest
    #'
    #' @param input_json the JSON input
    #' @return the instance of MailOrderRequest
    ...
    #' @return MailOrderRequest in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to MailOrderRequest and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`serviceType`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for MailOrderRequest: the required field `serviceType` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of MailOrderRequest
    toString = function() {
      self$toJSONString()
    },
...
# MailOrderRequest$unlock()
#
## Below is an example to define the print function
# MailOrderRequest$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# MailOrderRequest$lock()
