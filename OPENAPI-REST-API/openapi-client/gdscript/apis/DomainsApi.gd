...
func add_domain(
	...
	if not bzz_found_producible_mime:
		...
		error.identifier = "add_domain.headers.content_type"
...
func patch_domains(
	...
	if not bzz_found_producible_mime:
		var error := ApiError.new()
		error.identifier = "patch_domains.headers.content_type"
		error.message = "That endpoint only accepts %s as content type(s) and none are supported by this client."
		on_failure.call(error)
		return
	var bzz_mimes_produced_by_server := ['application/json']
...
# Operation postDomainSearch → POST /domains/search/{name}
...
func post_domain_search_threaded(
	# name: String = ""   Eg: name_example
	# The base domain name to search (for example `example` or `example.com`).
	name: String,
	on_success: Callable = Callable(),  # func(response: ApiResponse)
	...
	var bzz_thread := Thread.new()
	var bzz_callable := Callable(self, "post_domain_search")
	bzz_callable.bind(
		name,
		on_success,
		on_failure,
...
func put_domains(
	...
	if not bzz_found_producible_mime:
		var error := ApiError.new()
		error.identifier = "put_domains.headers.content_type"
		error.message = "That endpoint only accepts %s as content type(s) and none are supported by this client."
		on_failure.call(error)
		return
	var bzz_mimes_produced_by_server := ['application/json']
