...
------------- | ------------- | -------------
[**get_account_currencies**](#get_account_currencies) | **GET** `/account/currencies` | List enabled currency codes accepted for billing and preferences
[**get_account_locales**](#get_account_locales) | **GET** `/account/locales` | List supported UI locales with English and native display names
[**get_captcha**](#get_captcha) | **GET** `/captcha` | Fetch a base64 JPEG captcha challenge for human verification
[**get_countries**](#get_countries) | **GET** `/account/countries` | List enabled countries keyed by ISO-2/ISO-3/numeric code
[**get_info**](#get_info) | **GET** `/info` | Discover available modules, service packages, categories, and types
[**get_login_info**](#get_login_info) | **GET** `/login` | Fetch logo, captcha, language, and stats for rendering a login page
...
[**get_oauth_redirect**](#get_oauth_redirect) | **GET** `/oauth` | Begin OAuth login flow — redirect user to provider for authentication
[**get_timezones**](#get_timezones) | **GET** `/account/timezones` | List all PHP timezone identifiers usable on accounts and services
[**patch_oauth_two_factor**](#patch_oauth_two_factor) | **PATCH** `/oauth` | Submit 2FA code to finish OAuth login when account has 2FA enabled
[**ping_server**](#ping_server) | **GET** `/ping` | Liveness check — returns the JSON string \&quot;pong\&quot; to confirm API is up
[**post_oauth_callback**](#post_oauth_callback) | **POST** `/oauth` | Complete OAuth login by linking provider to existing or new account
[**submit_login**](#submit_login) | **POST** `/login` | Authenticate with email + password and return a session token
[**submit_signup**](#submit_signup) | **POST** `/signup` | Create a new customer account (email + password + captcha + ToS)

# **get_account_currencies**   { #get_account_currencies }
<a name="get_account_currencies"></a>

> `get_account_currencies( on_success: Callable, on_failure: Callable)`
...
# Invoke an endpoint
api.get_account_currencies(
	# On Success
	func(response):  # response is ApiResponse
		prints("Success!", "get_account_currencies", response)
...
# **get_account_locales**   { #get_account_locales }
<a name="get_account_locales"></a>

> `get_account_locales( on_success: Callable, on_failure: Callable)`
...
# Invoke an endpoint
api.get_account_locales(
	# On Success
	func(response):  # response is ApiResponse
		prints("Success!", "get_account_locales", response)
		assert(response.data is getAccountLocales_200_response_value)
		pass  # do things, make stuff
		,
	# On Error
	func(error):  # error is ApiError
