...
------------- | ------------- | -------------
[**add_vps**](#add_vps) | **POST** `/vps/order` | Place a new VPS order, create the invoice, and queue provisioning
[**delete_vps_backup**](#delete_vps_backup) | **DELETE** `/vps/{id}/backups` | Permanently delete a VPS backup file by name (irreversible)
[**do_vps_block_smtp**](#do_vps_block_smtp) | **GET** `/vps/{id}/block_smtp` | Block outbound SMTP (port 25) on the VPS to prevent spam/abuse
[**do_vps_disable_cd**](#do_vps_disable_cd) | **GET** `/vps/{id}/disable_cd` | Remove the virtual CD/DVD device entirely from the VPS configuration
[**do_vps_disable_quota**](#do_vps_disable_quota) | **GET** `/vps/{id}/disable_quota` | Disable per-user disk quota enforcement inside the VPS guest OS
[**do_vps_eject_cd**](#do_vps_eject_cd) | **GET** `/vps/{id}/eject_cd` | Eject the mounted ISO from the VPS virtual CD drive (keep the drive)
[**do_vps_enable_quota**](#do_vps_enable_quota) | **GET** `/vps/{id}/enable_quota` | Enable per-user disk quota enforcement inside the VPS guest OS
[**do_vps_restart**](#do_vps_restart) | **GET** `/vps/{id}/restart` | Reboot the VPS — preferred over stop+start for software changes
[**do_vps_start**](#do_vps_start) | **GET** `/vps/{id}/start` | Power on a stopped VPS instance
[**do_vps_stop**](#do_vps_stop) | **GET** `/vps/{id}/stop` | Power off a running VPS — billing continues until cancellation
[**download_vps_backup**](#download_vps_backup) | **PATCH** `/vps/{id}/backups` | Issue a 24-hour pre-signed URL to download a MinIO-backed VPS backup
[**get_new_vps**](#get_new_vps) | **GET** `/vps/order` | Get the VPS order catalog — platforms, OS templates, locations, pricing
[**get_vps_backup**](#get_vps_backup) | **GET** `/vps/{id}/backup` | Trigger a manual on-demand snapshot/backup of the VPS
[**get_vps_backups**](#get_vps_backups) | **GET** `/vps/{id}/backups` | List existing backups for the VPS across Swift, MinIO, and ZFS
[**get_vps_buy_hd_space**](#get_vps_buy_hd_space) | **GET** `/vps/{id}/buy_hd_space` | Get current additional disk size and per-GB monthly cost for the VPS
[**get_vps_buy_ip**](#get_vps_buy_ip) | **GET** `/vps/{id}/buy_ip` | Read current additional IPs, cap, and per-IP monthly cost for the VPS
[**get_vps_change_hostname**](#get_vps_change_hostname) | **GET** `/vps/{id}/change_hostname` | Read the VPS&#x27;s current hostname before changing it
[**get_vps_change_root_password**](#get_vps_change_root_password) | **GET** `/vps/{id}/change_root_password` | Pre-flight check before changing the VPS root password
[**get_vps_change_timezone**](#get_vps_change_timezone) | **GET** `/vps/{id}/change_timezone` | List IANA timezones supported by the VPS guest OS
[**get_vps_info**](#get_vps_info) | **GET** `/vps/{id}` | Get full details for one VPS — IPs, hostname, OS, slices, status, addons
[**get_vps_insert_cd**](#get_vps_insert_cd) | **GET** `/vps/{id}/insert_cd` | List ISO templates that can be mounted in the VPS virtual CD drive
[**get_vps_invoices**](#get_vps_invoices) | **GET** `/vps/{id}/invoices` | List all billing invoices associated with this specific VPS
[**get_vps_list**](#get_vps_list) | **GET** `/vps` | List all VPS services on the customer&#x27;s account
[**get_vps_reinstall_os**](#get_vps_reinstall_os) | **GET** `/vps/{id}/reinstall_os` | List OS templates compatible with this VPS&#x27;s hypervisor for reinstall
[**get_vps_reset_password**](#get_vps_reset_password) | **GET** `/vps/{id}/reset_password` | Pre-flight check before resetting the VPS root password to a random value
[**get_vps_reverse_dns**](#get_vps_reverse_dns) | **GET** `/vps/{id}/reverse_dns` | Read the current PTR (reverse-DNS) records for every IP on the VPS
[**get_vps_setup_vnc**](#get_vps_setup_vnc) | **GET** `/vps/{id}/setup_vnc` | Read current VNC console connection info for the VPS
[**get_vps_slices**](#get_vps_slices) | **GET** `/vps/{id}/slices` | Read current slice count, min/max range, and prorated per-slice upgrade cost
[**get_vps_traffic_usage**](#get_vps_traffic_usage) | **GET** `/vps/{id}/traffic_usage` | Read bandwidth traffic usage data for the VPS
[**get_vps_view_desktop**](#get_vps_view_desktop) | **GET** `/vps/{id}/view_desktop` | Read remote-desktop (RDP/HTML5) connection info for a Windows/GUI VPS
[**get_vps_welcome_email**](#get_vps_welcome_email) | **GET** `/vps/{id}/welcome_email` | Resend the welcome email containing VPS IP, hostname, and root credentials
[**post_vps_buy_hd_space**](#post_vps_buy_hd_space) | **POST** `/vps/{id}/buy_hd_space` | Buy or resize the VPS additional-disk addon and create a prorated invoice
[**post_vps_buy_ip**](#post_vps_buy_ip) | **POST** `/vps/{id}/buy_ip` | Purchase one additional IP for the VPS and create the invoice
[**post_vps_change_hostname**](#post_vps_change_hostname) | **POST** `/vps/{id}/change_hostname` | Rename the VPS hostname (OpenVZ/Virtuozzo only) and auto-set PTR for the primary IP
[**post_vps_change_root_password**](#post_vps_change_root_password) | **POST** `/vps/{id}/change_root_password` | Set a specific new root/Administrator password on the VPS
[**post_vps_change_timezone**](#post_vps_change_timezone) | **POST** `/vps/{id}/change_timezone` | Set the system timezone on the VPS guest OS
[**post_vps_change_webuzo_password**](#post_vps_change_webuzo_password) | **POST** `/vps/{id}/change_webuzo_password` | Rotate the Webuzo control panel admin password (re-auth required)
[**post_vps_insert_cd**](#post_vps_insert_cd) | **POST** `/vps/{id}/insert_cd` | Mount an ISO image in the VPS virtual CD drive from a URL
[**post_vps_reinstall_os**](#post_vps_reinstall_os) | **POST** `/vps/{id}/reinstall_os` | Reinstall the VPS OS (DESTRUCTIVE — wipes disk; requires re-auth)
[**post_vps_reset_password**](#post_vps_reset_password) | **POST** `/vps/{id}/reset_password` | Reset the VPS root password to a server-generated random value
[**post_vps_restore**](#post_vps_restore) | **POST** `/vps/{id}/restore` | Restore the VPS from a backup (DESTRUCTIVE — overwrites disk)
[**post_vps_reverse_dns**](#post_vps_reverse_dns) | **POST** `/vps/{id}/reverse_dns` | Bulk-update PTR (reverse-DNS) records for one or more VPS IPs
[**post_vps_setup_vnc**](#post_vps_setup_vnc) | **POST** `/vps/{id}/setup_vnc` | Provision or refresh the VNC console endpoint for the VPS
[**post_vps_slices**](#post_vps_slices) | **POST** `/vps/{id}/slices` | Upgrade or downgrade the VPS slice count (creates prorated invoice on upgrade)
[**post_vps_traffic_usage**](#post_vps_traffic_usage) | **POST** `/vps/{id}/traffic_usage` | Search/filter VPS bandwidth usage with custom criteria (reserved)
[**post_vps_view_desktop**](#post_vps_view_desktop) | **POST** `/vps/{id}/view_desktop` | Refresh the remote-desktop session connection info after IP/hostname changes
[**put_vps**](#put_vps) | **PUT** `/vps/order` | Validate a VPS order configuration and quote the cost — dry run, no charge
[**put_vps_buy_hd_space**](#put_vps_buy_hd_space) | **PUT** `/vps/{id}/buy_hd_space` | Preview cost to set additional VPS disk to a target GB size — dry run
[**update_vps_info**](#update_vps_info) | **POST** `/vps/{id}` | Update editable settings on a VPS service record
[**v_ps_cancel**](#v_ps_cancel) | **DELETE** `/vps/{id}` | Cancel a VPS service at the end of the current billing cycle
...
# **get_vps_backup**   { #get_vps_backup }
<a name="get_vps_backup"></a>

> `get_vps_backup(id: int, on_success: Callable, on_failure: Callable)`
...
api.get_vps_backup(
	...
	# On Success
	func(response):  # response is ApiResponse
		prints("Success!", "get_vps_backup", response)
		assert(response.data is QueueResponse)
		pass  # do things, make stuff
		,
	# On Error
...
# **get_vps_change_hostname**   { #get_vps_change_hostname }
<a name="get_vps_change_hostname"></a>

> `get_vps_change_hostname(id: int, on_success: Callable, on_failure: Callable)`
...
# Invoke an endpoint
api.get_vps_change_hostname(
	# id: int   Eg: 56
	# VPS ID number
	id,
	# On Success
	func(response):
		prints("Success!", "get_vps_change_hostname", response)
...
# **get_vps_change_root_password**   { #get_vps_change_root_password }
<a name="get_vps_change_root_password"></a>

> `get_vps_change_root_password(id: int, on_success: Callable, on_failure: Callable)`
...
# Invoke an endpoint
api.get_vps_change_root_password(
	# id: int   Eg: 56
	# VPS ID number
	id,
	# On Success
	func(response):
		prints("Success!", "get_vps_change_root_password", response)
...
# **get_vps_insert_cd**   { #get_vps_insert_cd }
<a name="get_vps_insert_cd"></a>

> `get_vps_insert_cd(id: int, on_success: Callable, on_failure: Callable)`
...
# Invoke an endpoint
api.get_vps_insert_cd(
	# id: int   Eg: 56
	# VPS ID number
	id,
	# On Success
	func(response):
		prints("Success!", "get_vps_insert_cd", response)
...
# **get_vps_reset_password**   { #get_vps_reset_password }
<a name="get_vps_reset_password"></a>

> `get_vps_reset_password(id: int, on_success: Callable, on_failure: Callable)`
...
# Invoke an endpoint
api.get_vps_reset_password(
	# id: int   Eg: 56
	# VPS ID number
	id,
	# On Success
	func(response):
		prints("Success!", "get_vps_reset_password", response)
...
# **post_vps_traffic_usage**   { #post_vps_traffic_usage }
<a name="post_vps_traffic_usage"></a>

> `post_vps_traffic_usage(id: int, on_success: Callable, on_failure: Callable)`
...
# Invoke an endpoint
api.post_vps_traffic_usage(
	# id: int   Eg: 56
	# VPS ID number
	id,
	# On Success
	func(response):
		prints("Success!", "post_vps_traffic_usage", response)
...
# **put_vps_buy_hd_space**   { #put_vps_buy_hd_space }
<a name="put_vps_buy_hd_space"></a>

> `put_vps_buy_hd_space(id: int, on_success: Callable, on_failure: Callable)`
...
# Invoke an endpoint
api.put_vps_buy_hd_space(
	# id: int   Eg: 56
	# VPS ID number
	id,
	# On Success
	func(response):
		prints("Success!", "put_vps_buy_hd_space", response)
