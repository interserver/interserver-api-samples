...
func add_website(
	...
	if not bzz_found_producible_mime:
		...
		error.identifier = "add_website.headers.content_type"
	...
	var bzz_body = null
	bzz_body = websiteOrderPostRequest
...
func put_websites(
	...
	if not bzz_found_producible_mime:
		var error := ApiError.new()
		error.identifier = "put_websites.headers.content_type"
		error.message = "That endpoint only accepts %s as content type(s) and none are supported by this client."
		on_failure.call(error)
		return
	var bzz_mimes_produced_by_server := ['application/json']
	...
	var bzz_body = null
	bzz_body = websiteOrderPutRequest
