...
------------- | ------------- | -------------
[**add_website**](#add_website) | **POST** `/websites/order` | Place a new webhosting order, create the invoice, and queue provisioning
[**get_new_website**](#get_new_website) | **GET** `/websites/order` | Read the webhosting order catalog — plans, packages, promo offers, pricing
[**get_website_buy_ip**](#get_website_buy_ip) | **GET** `/websites/{id}/buy_ip` | Read website IPs, current reverse DNS, and additional-IP pricing
[**get_website_info**](#get_website_info) | **GET** `/websites/{id}` | Read full configuration and status detail for one webhosting service
[**get_website_invoices**](#get_website_invoices) | **GET** `/websites/{id}/invoices` | List all billing invoices and recurring charges scoped to one website
[**get_website_list**](#get_website_list) | **GET** `/websites` | List the caller&#x27;s webhosting (cPanel/DirectAdmin/Plesk/Webuzo) services
[**get_websites_backups**](#get_websites_backups) | **GET** `/websites/{id}/backups` | List off-site cpmove backups stored in Swift — list or inline-download archive
[**get_websites_login**](#get_websites_login) | **GET** `/websites/{id}/login` | Get a one-time auto-login URL for the website&#x27;s control panel
[**get_websites_welcome_email**](#get_websites_welcome_email) | **GET** `/websites/{id}/welcome_email` | Resend the webhosting welcome email with control-panel credentials and URL
[**gett_website_reverse_dns**](#gett_website_reverse_dns) | **GET** `/websites/{id}/reverse_dns` | Read current reverse-DNS (PTR) records for the website&#x27;s IPs
[**post_website_buy_ip**](#post_website_buy_ip) | **POST** `/websites/{id}/buy_ip` | Buy an additional IP for the website OR update reverse DNS records
[**post_website_migration**](#post_website_migration) | **POST** `/websites/{id}/migration` | Submit a request for InterServer staff to migrate a website from another host
[**post_websites_reverse_dns**](#post_websites_reverse_dns) | **POST** `/websites/{id}/reverse_dns` | Bulk-update reverse-DNS (PTR) records for one or more website IPs
[**put_websites**](#put_websites) | **PUT** `/websites/order` | Validate a webhosting order and preview cost — dry run, no charge
[**update_website_info**](#update_website_info) | **POST** `/websites/{id}` | POST mutation hook for the website detail page (use dedicated ops where possible)
[**webhosting_cancel**](#webhosting_cancel) | **DELETE** `/websites/{id}` | Schedule termination of a webhosting service — wipes panel account at cycle end

# **add_website**   { #add_website }
<a name="add_website"></a>

> `add_website(websiteOrderPostRequest: WebsiteOrderPostRequest, on_success: Callable, on_failure: Callable)`
...
var websiteOrderPostRequest = WebsiteOrderPostRequest.new()
# … fill model websiteOrderPostRequest with data
...
> `put_websites(websiteOrderPutRequest: WebsiteOrderPutRequest, on_success: Callable, on_failure: Callable)`
...
var websiteOrderPutRequest = WebsiteOrderPutRequest.new()
# … fill model websiteOrderPutRequest with data
