...
------------- | ------------- | -------------
[**add_new_ticket**](#add_new_ticket) | **POST** `/tickets/new` | Open a new helpdesk ticket, optionally linked to a service and attachments
[**close_ticket**](#close_ticket) | **GET** `/tickets/{id}/close` | Close an open support ticket via simple GET request (no body required)
[**delete_ticket_info**](#delete_ticket_info) | **DELETE** `/tickets/{id}` | Close a customer ticket via DELETE verb (closes only, never destroys data)
[**get_new_ticket**](#get_new_ticket) | **GET** `/tickets/new` | Fetch services and product options to populate the new-ticket form
[**get_ticket_info**](#get_ticket_info) | **GET** `/tickets/{id}` | Get full ticket details including subject, status, and the reply thread
[**get_tickets_list**](#get_tickets_list) | **GET** `/tickets` | List the authenticated account&#x27;s support tickets with status and date filters
[**post_ticket_info**](#post_ticket_info) | **POST** `/tickets/{id}` | Append a reply (and optional attachment, server-access fields) to a ticket
[**post_tickets_list**](#post_tickets_list) | **POST** `/tickets` | Search the authenticated account&#x27;s tickets by subject, email, or mask ID
[**put_ticket_info**](#put_ticket_info) | **PUT** `/tickets/{id}` | Update a ticket&#x27;s properties such as subject or status (stub, not implemented)
[**reply_ticket**](#reply_ticket) | **POST** `/tickets/{id}/reply` | Post a simple text reply to an existing ticket thread (no attachments)
[**update_ticket_info**](#update_ticket_info) | **POST** `/tickets/{id}/update` | Update a ticket&#x27;s custom field values (server-access details, etc.)
