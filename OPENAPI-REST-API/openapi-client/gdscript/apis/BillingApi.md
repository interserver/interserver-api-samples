...
------------- | ------------- | -------------
[**add_billing_credit_card**](#add_billing_credit_card) | **POST** `/billing/creditcards` | Store a credit card on the account — may return a verification flow
[**add_billing_prepay**](#add_billing_prepay) | **POST** `/billing/prepays` | Create a prepay deposit and return an invoice id to fund it
[**delete_billing_credit_card**](#delete_billing_credit_card) | **DELETE** `/billing/creditcards/{id}` | Remove a stored credit card from the account
[**delete_billing_invoice**](#delete_billing_invoice) | **DELETE** `/billing/invoices/{id}` | Cancel a pending unpaid invoice — and its pending service or repeat invoice
[**delete_billing_prepay**](#delete_billing_prepay) | **DELETE** `/billing/prepays/{id}` | Delete an unfunded prepay or strip its unpaid funding invoices
[**get_affiliate_banners**](#get_affiliate_banners) | **GET** `/affiliate/banners` | List affiliate banner image assets with filename and dimensions
[**get_affiliate_download**](#get_affiliate_download) | **GET** `/affiliate/download` | Export the affiliate signup report as CSV, XLS, XLSX, or PDF file download
[**get_affiliate_rich_report**](#get_affiliate_rich_report) | **GET** `/affiliate/rich_report` | Read a combined affiliate performance summary (HTML payload)
[**get_affiliate_sales_graph**](#get_affiliate_sales_graph) | **GET** `/affiliate/sales_graph` | Read aggregated affiliate sales time-series (monthly buckets) for chart rendering
[**get_affiliate_signups**](#get_affiliate_signups) | **GET** `/affiliate/signups` | Read affiliate signup stats and per-customer conversion data
[**get_affiliate_traffic_graph**](#get_affiliate_traffic_graph) | **GET** `/affiliate/traffic_graph` | Read aggregated affiliate referral click/visit time-series for chart rendering
[**get_affiliate_web_traffic**](#get_affiliate_web_traffic) | **GET** `/affiliate/web_traffic` | List the 20 most recent affiliate referral visits with IP, referrer, timestamp
[**get_billing_cart**](#get_billing_cart) | **GET** `/billing/cart` | Read the current shopping cart contents, totals, and available payment methods
[**get_billing_credit_card_verify**](#get_billing_credit_card_verify) | **GET** `/billing/creditcards/{id}/verify` | Probe whether a stored card still needs micro-charge verification
[**get_billing_invoice**](#get_billing_invoice) | **GET** `/billing/invoices/{id}` | Read full invoice detail — line items, totals, paid status, customer info
[**get_billing_invoices**](#get_billing_invoices) | **GET** `/billing/invoices` | List every invoice on the account with summary totals and paid/unpaid status
[**get_billing_pre_pays**](#get_billing_pre_pays) | **GET** `/billing/prepays` | List prepay deposits on the account — remaining balance and auto-use flags
[**initiate_payment**](#initiate_payment) | **GET** `/billing/pay/{method}/{invoices}` | Pay invoices through the chosen gateway — returns the next-step action
[**patch_billing_credit_card_verify**](#patch_billing_credit_card_verify) | **PATCH** `/billing/creditcards/{id}/verify` | Place two micro-charges on the card to start CVV verification (step 1 of 2)
[**post_billing_credit_card_verify**](#post_billing_credit_card_verify) | **POST** `/billing/creditcards/{id}/verify` | Submit two micro-charge amounts to finalize card verification (step 2 of 2)
[**update_affiliate_dock_setup**](#update_affiliate_dock_setup) | **POST** `/affiliate/dock_setup` | Configure the affiliate landing dock title, description, and referrer coupon
[**update_affiliate_payment_setup**](#update_affiliate_payment_setup) | **POST** `/affiliate/payment_setup` | Configure how affiliate commissions get paid out (PayPal or internal prepay)
[**update_billing_credit_card**](#update_billing_credit_card) | **POST** `/billing/creditcards/{id}` | Refresh stored card expiration and re-trigger MaxMind fraud scoring
[**update_billing_payment_method**](#update_billing_payment_method) | **POST** `/billing/payment_method` | Set the account&#x27;s default payment method for recurring/auto charges
...
# **get_affiliate_download**   { #get_affiliate_download }
<a name="get_affiliate_download"></a>

> `get_affiliate_download(st = "",ex = "",year = null, on_success: Callable, on_failure: Callable)`
...
# Invoke an endpoint
api.get_affiliate_download(
	# st: String = ""   Eg: st_example
	# Filter by status.
	st,
	# ex: String = ""   Eg: ex_example
	...
	# On Success
	func(response):
		prints("Success!", "get_affiliate_download", response)
...
# **get_affiliate_signups**   { #get_affiliate_signups }
<a name="get_affiliate_signups"></a>

> `get_affiliate_signups(st = "", on_success: Callable, on_failure: Callable)`
...
# Invoke an endpoint
api.get_affiliate_signups(
	# st: String = ""   Eg: st_example
	# Filter signups by status. Use `default` to show all or pass a specific status value to narrow results.
	st,
	# On Success
	func(response):  # response is ApiResponse
		prints("Success!", "get_affiliate_signups", response)
		assert(response.data is getAffiliateSignups_200_response)
		pass  # do things, make stuff
		,
	# On Error
	func(error):  # error is ApiError
...
# **patch_billing_credit_card_verify**   { #patch_billing_credit_card_verify }
<a name="patch_billing_credit_card_verify"></a>

> `patch_billing_credit_card_verify(id: int,patchBillingCreditCardVerifyRequest: PatchBillingCreditCardVerifyRequest, on_success: Callable, on_failure: Callable)`
...
var patchBillingCreditCardVerifyRequest = PatchBillingCreditCardVerifyRequest.new()
# … fill model patchBillingCreditCardVerifyRequest with data

# Invoke an endpoint
api.patch_billing_credit_card_verify(
	# id: int   Eg: 56
	# The credit card ID to verify. Use the ID returned from `POST /billing/creditcards`.
	id,
	...
	# On Success
	func(response):  # response is ApiResponse
		prints("Success!", "patch_billing_credit_card_verify", response)
		assert(response.data is SuccessTextResponse)
		pass  # do things, make stuff
		,
	# On Error
