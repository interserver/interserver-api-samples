# InterServerAPI::SSLCertificatesApi

## Load the API package
```perl
use InterServerAPI::Object::SSLCertificatesApi;
```
...
# **add_ssl**
> ServiceOrderPostResponse add_ssl(ssl_order_request => $ssl_order_request)
...
my $api_instance = InterServerAPI::SSLCertificatesApi->new(
...
my $ssl_order_request = InterServerAPI::Object::SslOrderRequest->new(); # SslOrderRequest | 

eval {
    my $result = $api_instance->add_ssl(ssl_order_request => $ssl_order_request);
    print Dumper($result);
};
if ($@) {
...
eval {
    $api_instance->put_ssl(ssl_order_request => $ssl_order_request);
};
if ($@) {
