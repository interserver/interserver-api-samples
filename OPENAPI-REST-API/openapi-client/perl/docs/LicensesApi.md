# InterServerAPI::LicensesApi

## Load the API package
```perl
use InterServerAPI::Object::LicensesApi;
```
...
# **add_license**
> ServiceOrderPostResponse add_license(license_order_request => $license_order_request)
...
my $api_instance = InterServerAPI::LicensesApi->new(
...
my $license_order_request = InterServerAPI::Object::LicenseOrderRequest->new(); # LicenseOrderRequest | 

eval {
    my $result = $api_instance->add_license(license_order_request => $license_order_request);
    print Dumper($result);
};
if ($@) {
...
my $id = 56; # int | The license service ID. Use `license_id` from `GET /licenses`.
my $ip_object = InterServerAPI::Object::IpObject->new(); # IpObject | 
...
eval {
    $api_instance->put_licenses(license_order_request => $license_order_request);
};
if ($@) {
