# InterServerAPI::VPSApi

## Load the API package
```perl
use InterServerAPI::Object::VPSApi;
```
...
my $api_instance = InterServerAPI::VPSApi->new(
...
my $vps_order_post_request = InterServerAPI::Object::VpsOrderPostRequest->new(); # VpsOrderPostRequest | 
...
my $id = 56; # int | VPS ID number
my $download_qs_backup_request = InterServerAPI::Object::DownloadQsBackupRequest->new(); # DownloadQsBackupRequest | 
my $all = "all_example"; # string | Set to `1` to list all backups across all services, not just the ones for the given VPS.
...
# **get_vps_backup**
> QueueResponse get_vps_backup(id => $id)
...
eval {
    my $result = $api_instance->get_vps_backup(id => $id);
    print Dumper($result);
};
if ($@) {
    warn "Exception when calling VPSApi->get_vps_backup: $@\n";
}
```
...
eval {
    $api_instance->get_vps_change_hostname(id => $id);
};
if ($@) {
    warn "Exception when calling VPSApi->get_vps_change_hostname: $@\n";
}
```
...
> get_vps_change_root_password(id => $id)
...
eval {
    $api_instance->get_vps_change_root_password(id => $id);
};
if ($@) {
    warn "Exception when calling VPSApi->get_vps_change_root_password: $@\n";
}
```
...
eval {
    $api_instance->get_vps_insert_cd(id => $id);
};
if ($@) {
    warn "Exception when calling VPSApi->get_vps_insert_cd: $@\n";
}
```
...
eval {
    $api_instance->get_vps_reset_password(id => $id);
};
if ($@) {
    warn "Exception when calling VPSApi->get_vps_reset_password: $@\n";
}
```
...
my $id = 56; # int | VPS ID number
my $restore_request = InterServerAPI::Object::RestoreRequest->new(); # RestoreRequest | VPS Restore request
...
eval {
    $api_instance->post_vps_traffic_usage(id => $id);
};
if ($@) {
    warn "Exception when calling VPSApi->post_vps_traffic_usage: $@\n";
}
```
...
my $vps_order_put_request = InterServerAPI::Object::VpsOrderPutRequest->new(); # VpsOrderPutRequest | 
...
eval {
    $api_instance->put_vps_buy_hd_space(id => $id);
};
if ($@) {
    warn "Exception when calling VPSApi->put_vps_buy_hd_space: $@\n";
}
```
