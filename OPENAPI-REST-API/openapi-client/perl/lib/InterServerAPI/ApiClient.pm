...
#
package InterServerAPI::ApiClient;
...
use InterServerAPI::Configuration;

sub new {
    my $class = shift;

    my $config;
    if ( $_[0] && ref $_[0] && ref $_[0] eq 'InterServerAPI::Configuration' ) {
        $config = $_[0];
    } else {
        $config = InterServerAPI::Configuration->new(@_);
    }
...
sub deserialize
{
    ...
    if (not defined $data) {
        ...
        my $_instance = use_module("InterServerAPI::Object::$class")->new;
        if (ref $data eq "HASH") {
