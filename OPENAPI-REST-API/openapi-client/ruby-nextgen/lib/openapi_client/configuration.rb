...
module OpenapiClient
  class Configuration
    attr_accessor :base_url, :timeout, :logger, :debugging, :query_array_encoding, :api_key, :api_key, :api_key

    def initialize(base_url: nil, **options)
      @base_url = base_url || 'https://my.interserver.net/apiv2'
      @timeout = 60
    ...
    def apply_auth(headers, query, auth_names)
      headers['X-API-KEY'] = api_key if api_key && auth_names.include?('apiKeyAuth')
      headers['sessionid'] = api_key if api_key && auth_names.include?('sessionIdHeaderAuth')
      if api_key && auth_names.include?('sessionIdCookieAuth')
