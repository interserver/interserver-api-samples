...
api_instance = InterServerClient::DNSApi.new
domain = 'domain_example' # String | The domain name.
ip = 'ip_example' # String | IP Address to point the domain to.

begin
  # Create a new authoritative DNS zone seeded with apex A + NS + SOA records
  result = api_instance.add_dns_domain(domain, ip)
  p result
rescue InterServerClient::ApiError => e
  puts "Error when calling DNSApi->add_dns_domain: #{e}"
end
```

#### Using the add_dns_domain_with_http_info variant

This returns an Array which contains the response data, status code and headers.

> <Array(<SuccessTextResponse>, Integer, Hash)> add_dns_domain_with_http_info(domain, ip)

```ruby
begin
  # Create a new authoritative DNS zone seeded with apex A + NS + SOA records
  data, status_code, headers = api_instance.add_dns_domain_with_http_info(domain, ip)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <SuccessTextResponse>
rescue InterServerClient::ApiError => e
  puts "Error when calling DNSApi->add_dns_domain_with_http_info: #{e}"
end
```
...
> add_dns_record(id, name, type, content, opts)
...
type = InterServerClient::DnsRecordType::A # DnsRecordType | 
content = 'content_example' # String | Content of record
opts = {
  ttl: 56, # Integer | Time-to-live
  prio: 56 # Integer | Priority
}

begin
  # Add a DNS record (A, AAAA, MX, TXT, CNAME, NS, SRV, CAA, ...) to a zone
  api_instance.add_dns_record(id, name, type, content, opts)
rescue InterServerClient::ApiError => e
  puts "Error when calling DNSApi->add_dns_record: #{e}"
end
```

#### Using the add_dns_record_with_http_info variant

This returns an Array which contains the response data (`nil` in this case), status code and headers.

> <Array(nil, Integer, Hash)> add_dns_record_with_http_info(id, name, type, content, opts)

```ruby
begin
  # Add a DNS record (A, AAAA, MX, TXT, CNAME, NS, SRV, CAA, ...) to a zone
  data, status_code, headers = api_instance.add_dns_record_with_http_info(id, name, type, content, opts)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => nil
rescue InterServerClient::ApiError => e
  puts "Error when calling DNSApi->add_dns_record_with_http_info: #{e}"
end
```
...
> <Array(<SuccessTextResponse>, Integer, Hash)> delete_dns_domain_with_http_info(id)

```ruby
begin
  # Permanently delete a DNS zone and every record it contains
  data, status_code, headers = api_instance.delete_dns_domain_with_http_info(id)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <SuccessTextResponse>
rescue InterServerClient::ApiError => e
...
begin
  ...
  puts "Error when calling DNSApi->delete_dns_record: #{e}"
end
```
...
> <Array(<SuccessTextResponse>, Integer, Hash)> delete_dns_record_with_http_info(domain_id, record_id)

```ruby
begin
  # Permanently delete one DNS record from a zone — zone itself is preserved
  data, status_code, headers = api_instance.delete_dns_record_with_http_info(domain_id, record_id)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <SuccessTextResponse>
rescue InterServerClient::ApiError => e
  puts "Error when calling DNSApi->delete_dns_record_with_http_info: #{e}"
end
```
...
> <Array(<Array<DnsRecord>>, Integer, Hash)> get_dns_domain_with_http_info(id)

```ruby
begin
  # List every DNS record in one zone with the IDs needed to edit or delete them
  data, status_code, headers = api_instance.get_dns_domain_with_http_info(id)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <Array<DnsRecord>>
rescue InterServerClient::ApiError => e
...
begin
  ...
  puts "Error when calling DNSApi->get_dns_list: #{e}"
end
```

#### Using the get_dns_list_with_http_info variant

This returns an Array which contains the response data, status code and headers.

> <Array(<Array<DnsListItem>>, Integer, Hash)> get_dns_list_with_http_info

```ruby
begin
  # List DNS zones hosted on the account with each zone's apex A-record IP
  data, status_code, headers = api_instance.get_dns_list_with_http_info
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <Array<DnsListItem>>
rescue InterServerClient::ApiError => e
  puts "Error when calling DNSApi->get_dns_list_with_http_info: #{e}"
end
```
...
> <SuccessTextResponse> update_dns_record(domain_id, record_id, opts)
...
begin
  # Replace values on an existing DNS record (name, type, content, ttl, priority)
  result = api_instance.update_dns_record(domain_id, record_id, opts)
  p result
rescue InterServerClient::ApiError => e
  puts "Error when calling DNSApi->update_dns_record: #{e}"
end
```
...
> <Array(<SuccessTextResponse>, Integer, Hash)> update_dns_record_with_http_info(domain_id, record_id, opts)

```ruby
begin
  # Replace values on an existing DNS record (name, type, content, ttl, priority)
  data, status_code, headers = api_instance.update_dns_record_with_http_info(domain_id, record_id, opts)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <SuccessTextResponse>
rescue InterServerClient::ApiError => e
  puts "Error when calling DNSApi->update_dns_record_with_http_info: #{e}"
end
```
