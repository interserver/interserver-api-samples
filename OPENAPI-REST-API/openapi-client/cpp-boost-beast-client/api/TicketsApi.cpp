...
TicketsApiException::TicketsApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status TicketsApiException::getStatus() const
{
    return m_status;
}

const char* TicketsApiException::what() const noexcept
{
    return m_what.c_str();
}

std::shared_ptr<TicketNewResponse>
TicketsApi::addNewTicket(
    const std::shared_ptr<TicketNew>& ticketNew) {
    std::string serializedRequestBody;
    std::string path = m_context + "/tickets/new";
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(ticketNew));
    } else {
    ...
    std::shared_ptr<TicketNewResponse> deserializedResponse = std::make_shared<TicketNewResponse>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<TicketNewResponse>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw TicketsApiException(statusCode, "A successful response after creating a ticket");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw TicketsApiException(statusCode, "Unauthorized");
    }
    throw TicketsApiException(statusCode, "Unexpected HTTP status code");
}
void
TicketsApi::getNewTicket(
    ) {
    std::string serializedRequestBody;
    std::string path = m_context + "/tickets/new";
    std::map<std::string, std::string> headers;
...
std::shared_ptr<CloseTicketResponseSchema>
TicketsApi::closeTicket(
    const std::string& id) {
    std::string serializedRequestBody;
    std::string path = m_context + "/tickets/{id}/close";
    std::map<std::string, std::string> headers;
    // path params
    ...
    std::shared_ptr<CloseTicketResponseSchema> deserializedResponse = std::make_shared<CloseTicketResponseSchema>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<CloseTicketResponseSchema>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::shared_ptr<ViewTicketResponse>
TicketsApi::deleteTicketInfo(
    const double& id) {
    std::string serializedRequestBody;
    std::string path = m_context + "/tickets/{id}";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "id", id);
    ...
    std::shared_ptr<ViewTicketResponse> deserializedResponse = std::make_shared<ViewTicketResponse>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<ViewTicketResponse>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::shared_ptr<Tickets>
TicketsApi::getTicketsList(
    const boost::optional<int32_t>& page, const boost::optional<std::string>& period, const boost::optional<std::string>& view) {
    std::string serializedRequestBody;
    std::string path = m_context + "/tickets";
    std::map<std::string, std::string> headers;
    // query params
    std::stringstream queryParameterStream;
    const char* queryParameterSeparator = "?";
    if (page) {
        appendQueryParameter(
            ...
            queryParameterSeparator,
            "page",
            serializeQueryParameterValue(*page));
    }
    if (period) {
        appendQueryParameter(
            queryParameterStream,
            queryParameterSeparator,
            "period",
            serializeQueryParameterValue(*period));
    }
    if (view) {
        appendQueryParameter(
            queryParameterStream,
            queryParameterSeparator,
            "view",
            serializeQueryParameterValue(*view));
    }
    ...
    std::shared_ptr<Tickets> deserializedResponse = std::make_shared<Tickets>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Tickets>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::shared_ptr<ReplyTicketResponseSchema>
TicketsApi::replyTicket(
    const double& id, const std::shared_ptr<ReplyTicketRequest>& replyTicketRequest) {
    std::string serializedRequestBody;
    std::string path = m_context + "/tickets/{id}/reply";
    std::map<std::string, std::string> headers;
    static const std::vector<std::string> contentTypes{ "application/json","multipart/form-data", };
    std::string requestContentType = selectPreferredContentType(contentTypes);
    headers["Content-Type"] = requestContentType;
    // Body params
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(replyTicketRequest));
    } else {
        throw std::invalid_argument("Content type '" + requestContentType + "' does not support structured request bodies");
    }
    // path params
    ...
    std::shared_ptr<ReplyTicketResponseSchema> deserializedResponse = std::make_shared<ReplyTicketResponseSchema>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<ReplyTicketResponseSchema>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::shared_ptr<UpdateTicketResponseSchema>
TicketsApi::updateTicketInfo(
    const double& id, const std::shared_ptr<UpdateTicket>& updateTicket) {
    std::string serializedRequestBody;
    std::string path = m_context + "/tickets/{id}/update";
    std::map<std::string, std::string> headers;
    static const std::vector<std::string> contentTypes{ "application/json","multipart/form-data", };
    std::string requestContentType = selectPreferredContentType(contentTypes);
    headers["Content-Type"] = requestContentType;
    // Body params
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(updateTicket));
    } else {
        throw std::invalid_argument("Content type '" + requestContentType + "' does not support structured request bodies");
    }
    // path params
    ...
    std::shared_ptr<UpdateTicketResponseSchema> deserializedResponse = std::make_shared<UpdateTicketResponseSchema>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<UpdateTicketResponseSchema>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::string TicketsApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void TicketsApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void TicketsApi::handleStdException(const std::exception&) {
    throw;
}

void TicketsApi::handleUncaughtException() {
    throw;
}
