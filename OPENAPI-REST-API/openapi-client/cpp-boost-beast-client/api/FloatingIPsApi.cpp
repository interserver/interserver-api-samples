...
FloatingIPsApiException::FloatingIPsApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status FloatingIPsApiException::getStatus() const
{
    return m_status;
}

const char* FloatingIPsApiException::what() const noexcept
{
    return m_what.c_str();
}

std::shared_ptr<ServiceOrderPostResponse>
FloatingIPsApi::addFloatingIp(
    const std::shared_ptr<FloatingIpOrderRequest>& floatingIpOrderRequest) {
    std::string serializedRequestBody;
    std::string path = m_context + "/floating_ips/order";
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(floatingIpOrderRequest));
    } else {
    ...
    if (statusCode == boost::beast::http::status(401)) {
        throw FloatingIPsApiException(statusCode, "Unauthorized");
    }
    throw FloatingIPsApiException(statusCode, "Unexpected HTTP status code");
}
boost::json::value
FloatingIPsApi::getNewFloatingIp(
    ) {
    std::string serializedRequestBody;
    std::string path = m_context + "/floating_ips/order";
    std::map<std::string, std::string> headers;
...
std::shared_ptr<Floating_ipsCancel_200_response>
FloatingIPsApi::floatingIpsCancel(
    const int32_t& id) {
    std::string serializedRequestBody;
    std::string path = m_context + "/floating_ips/{id}";
    std::map<std::string, std::string> headers;
    // path params
    ...
    std::shared_ptr<Floating_ipsCancel_200_response> deserializedResponse = std::make_shared<Floating_ipsCancel_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Floating_ipsCancel_200_response>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
boost::json::value
FloatingIPsApi::getFloatingIpInfo(
    const int32_t& id) {
    std::string serializedRequestBody;
    std::string path = m_context + "/floating_ips/{id}";
    std::map<std::string, std::string> headers;
...
FloatingIPsApi::updateFloatingIpInfo(
    const std::string& id) {
    std::string serializedRequestBody;
    std::string path = m_context + "/floating_ips/{id}";
    std::map<std::string, std::string> headers;
...
FloatingIPsApi::getFloatingIpInvoices(
    const int32_t& id) {
    std::string serializedRequestBody;
    std::string path = m_context + "/floating_ips/{id}/invoices";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "id", id);
...
FloatingIPsApi::getFloatingIpsList(
    ) {
    std::string serializedRequestBody;
    std::string path = m_context + "/floating_ips";
    std::map<std::string, std::string> headers;
    ...
    std::vector<boost::json::value> deserializedResponse = std::vector<boost::json::value>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::vector<boost::json::value>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::shared_ptr<SuccessTextResponse>
FloatingIPsApi::getFloatingIpsWelcomeEmail(
    const int32_t& id) {
    std::string serializedRequestBody;
    std::string path = m_context + "/floating_ips/{id}/welcome_email";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "id", id);
...
std::shared_ptr<SuccessTextResponse>
FloatingIPsApi::postFloatingIpsChangeIp(
    const int32_t& id, const std::string& ip) {
    std::string serializedRequestBody;
    std::string path = m_context + "/floating_ips/{id}/change_ip";
    std::map<std::string, std::string> headers;
...
std::string FloatingIPsApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void FloatingIPsApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void FloatingIPsApi::handleStdException(const std::exception&) {
    throw;
}

void FloatingIPsApi::handleUncaughtException() {
    throw;
}
