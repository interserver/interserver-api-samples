...
DNSApiException::DNSApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status DNSApiException::getStatus() const
{
    return m_status;
}

const char* DNSApiException::what() const noexcept
{
    return m_what.c_str();
}
...
DNSApi::addDnsDomain(
    ...
    if (statusCode == boost::beast::http::status(401)) {
        throw DNSApiException(statusCode, "Unauthorized");
    }
    throw DNSApiException(statusCode, "Unexpected HTTP status code");
}
std::vector<std::shared_ptr<DnsListItem>>
DNSApi::getDnsList(
    ) {
    std::string serializedRequestBody;
    std::string path = m_context + "/dns";
    ...
    std::vector<std::shared_ptr<DnsListItem>> deserializedResponse = std::vector<std::shared_ptr<DnsListItem>>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::vector<std::shared_ptr<DnsListItem>>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
void
DNSApi::addDnsRecord(
    const std::string& id, const std::string& name, const std::shared_ptr<DnsRecordType>& type, const std::string& content, const int32_t& ttl, const int32_t& prio) {
    std::string serializedRequestBody;
    std::string path = m_context + "/dns/{id}";
    std::map<std::string, std::string> headers;
    static const std::vector<std::string> contentTypes{ "multipart/form-data","application/json", };
    std::string requestContentType = selectPreferredContentType(contentTypes);
    headers["Content-Type"] = requestContentType;
    ...
    formParameters.emplace_back(
        "type",
        toFormParameterValue(type),
        false);
    formParameters.emplace_back(
        "content",
        toFormParameterValue(content),
        false);
    formParameters.emplace_back(
        "ttl",
        toFormParameterValue(ttl),
        false);
    formParameters.emplace_back(
        "prio",
        toFormParameterValue(prio),
        false);
    if (normalizeMediaType(requestContentType) == "multipart/form-data") {
        const std::string multipartBoundary = selectMultipartBoundary(formParameters);
        headers["Content-Type"] = requestContentType + "; boundary=" + multipartBoundary;
...
std::vector<std::shared_ptr<DnsRecord>>
DNSApi::getDnsDomain(
    const int32_t& id) {
    std::string serializedRequestBody;
    std::string path = m_context + "/dns/{id}";
    ...
    std::vector<std::shared_ptr<DnsRecord>> deserializedResponse = std::vector<std::shared_ptr<DnsRecord>>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::vector<std::shared_ptr<DnsRecord>>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::shared_ptr<SuccessTextResponse>
DNSApi::deleteDnsRecord(
    const int32_t& domainId, const int32_t& recordId) {
    std::string serializedRequestBody;
    std::string path = m_context + "/dns/{domainId}/{recordId}";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "domainId", domainId);
    replacePathParameter(path, "recordId", recordId);
...
std::shared_ptr<SuccessTextResponse>
DNSApi::updateDnsRecord(
    const int32_t& domainId, const int32_t& recordId, const std::string& name, const std::shared_ptr<DnsRecordType>& type, const std::string& content, const std::string& ttl, const std::string& prio, const std::string& disabled, const std::string& ordername, const std::string& auth) {
    std::string serializedRequestBody;
    std::string path = m_context + "/dns/{domainId}/{recordId}";
    std::map<std::string, std::string> headers;
    static const std::vector<std::string> contentTypes{ "multipart/form-data","application/json", };
    ...
    formParameters.emplace_back(
        "disabled",
        toFormParameterValue(disabled),
        false);
    formParameters.emplace_back(
        "ordername",
        toFormParameterValue(ordername),
        false);
    formParameters.emplace_back(
        "auth",
        toFormParameterValue(auth),
        false);
    if (normalizeMediaType(requestContentType) == "multipart/form-data") {
        const std::string multipartBoundary = selectMultipartBoundary(formParameters);
        headers["Content-Type"] = requestContentType + "; boundary=" + multipartBoundary;
...
std::string DNSApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void DNSApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void DNSApi::handleStdException(const std::exception&) {
    throw;
}

void DNSApi::handleUncaughtException() {
    throw;
}
