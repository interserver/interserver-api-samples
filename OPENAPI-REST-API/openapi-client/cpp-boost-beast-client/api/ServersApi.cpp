...
ServersApiException::ServersApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status ServersApiException::getStatus() const
{
    return m_status;
}

const char* ServersApiException::what() const noexcept
{
    return m_what.c_str();
}

std::shared_ptr<AddServer_200_response>
ServersApi::addServer(
    const std::shared_ptr<ServerOrderPostRequest>& serverOrderPostRequest) {
    std::string serializedRequestBody;
    std::string path = m_context + "/servers/order";
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(serverOrderPostRequest));
    } else {
    ...
    std::shared_ptr<AddServer_200_response> deserializedResponse = std::make_shared<AddServer_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<AddServer_200_response>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(401)) {
        throw ServersApiException(statusCode, "Unauthorized");
    }
    throw ServersApiException(statusCode, "Unexpected HTTP status code");
}
std::shared_ptr<ServerOrder>
ServersApi::getNewServer(
    ) {
    std::string serializedRequestBody;
    std::string path = m_context + "/servers/order";
    std::map<std::string, std::string> headers;
    ...
    std::shared_ptr<ServerOrder> deserializedResponse = std::make_shared<ServerOrder>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<ServerOrder>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::shared_ptr<BuyItNowServerOrder_200_response>
ServersApi::buyItNowServerOrder(
    ) {
    std::string serializedRequestBody;
    std::string path = m_context + "/servers/order/buy_now_server";
    std::map<std::string, std::string> headers;
    ...
    std::shared_ptr<BuyItNowServerOrder_200_response> deserializedResponse = std::make_shared<BuyItNowServerOrder_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<BuyItNowServerOrder_200_response>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::shared_ptr<ServersBuyNowResponse>
ServersApi::placeBuyNowServer(
    const std::shared_ptr<PlaceBuyNowServer_request>& placeBuyNowServerRequest) {
    std::string serializedRequestBody;
    std::string path = m_context + "/servers/order/buy_now_server";
    std::map<std::string, std::string> headers;
    static const std::vector<std::string> contentTypes{ "application/json", };
    ...
    // Body params
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(placeBuyNowServerRequest));
    } else {
        throw std::invalid_argument("Content type '" + requestContentType + "' does not support structured request bodies");
    }
    ...
    std::shared_ptr<ServersBuyNowResponse> deserializedResponse = std::make_shared<ServersBuyNowResponse>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<ServersBuyNowResponse>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw ServersApiException(statusCode, "Order validation failed.");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw ServersApiException(statusCode, "Unauthorized");
    }
...
ServersApi::getServerInfo(
    const int32_t& id) {
    std::string serializedRequestBody;
    std::string path = m_context + "/servers/{id}";
    std::map<std::string, std::string> headers;
    // path params
    ...
    std::shared_ptr<Server> deserializedResponse = std::make_shared<Server>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<Server>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::shared_ptr<ServersCancel_200_response>
ServersApi::serversCancel(
    const int32_t& id) {
    std::string serializedRequestBody;
    std::string path = m_context + "/servers/{id}";
    std::map<std::string, std::string> headers;
    ...
    std::shared_ptr<ServersCancel_200_response> deserializedResponse = std::make_shared<ServersCancel_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<ServersCancel_200_response>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
ServersApi::getServerInvoices(
    const int32_t& id) {
    std::string serializedRequestBody;
    std::string path = m_context + "/servers/{id}/invoices";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "id", id);
...
std::vector<std::shared_ptr<ServerRow>>
ServersApi::getServerList(
    ) {
    std::string serializedRequestBody;
    std::string path = m_context + "/servers";
    std::map<std::string, std::string> headers;
    ...
    std::vector<std::shared_ptr<ServerRow>> deserializedResponse = std::vector<std::shared_ptr<ServerRow>>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::vector<std::shared_ptr<ServerRow>>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
ServersApi::getServerReverseDns(
    const int32_t& id) {
    std::string serializedRequestBody;
    std::string path = m_context + "/servers/{id}/reverse_dns";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "id", id);
...
ServersApi::postServerReverseDns(
...
std::shared_ptr<SuccessTextResponse>
ServersApi::getServersWelcomeEmail(
    const int32_t& id) {
    std::string serializedRequestBody;
    std::string path = m_context + "/servers/{id}/welcome_email";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "id", id);
...
std::shared_ptr<ServerBulkIpmiPowerResponse>
ServersApi::serverBulkIpmiPowerGet(
    const std::string& ids) {
    std::string serializedRequestBody;
    std::string path = m_context + "/servers/bulk/ipmi_power";
    std::map<std::string, std::string> headers;
    // query params
    std::stringstream queryParameterStream;
    const char* queryParameterSeparator = "?";
        appendQueryParameter(
            ...
            queryParameterSeparator,
            "ids",
            serializeQueryParameterValue(ids));
    ...
    std::shared_ptr<ServerBulkIpmiPowerResponse> deserializedResponse = std::make_shared<ServerBulkIpmiPowerResponse>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<ServerBulkIpmiPowerResponse>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw ServersApiException(statusCode, "The specified resource was not found");
    }
    if (statusCode == boost::beast::http::status(401)) {
        throw ServersApiException(statusCode, "Unauthorized");
    }
...
std::shared_ptr<ServerIpmiLiveInfo>
ServersApi::serverIpmiLiveGet(
    const int32_t& id) {
    std::string serializedRequestBody;
    std::string path = m_context + "/servers/{id}/ipmi_live";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "id", id);
    ...
    std::shared_ptr<ServerIpmiLiveInfo> deserializedResponse = std::make_shared<ServerIpmiLiveInfo>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<ServerIpmiLiveInfo>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::shared_ptr<ServerIpmiLiveInfo>
ServersApi::serverIpmiLivePost(
    const int32_t& id, const std::string& ip, const int32_t& asset) {
    std::string serializedRequestBody;
    std::string path = m_context + "/servers/{id}/ipmi_live";
    std::map<std::string, std::string> headers;
    ...
    formParameters.emplace_back(
        "ip",
        toFormParameterValue(ip),
        false);
    formParameters.emplace_back(
        "asset",
        toFormParameterValue(asset),
        false);
...
std::shared_ptr<TextResponse>
ServersApi::serverIpmiPowerGet(
    const int32_t& id) {
    std::string serializedRequestBody;
    std::string path = m_context + "/servers/{id}/ipmi_power";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "id", id);
...
std::shared_ptr<TextResponse>
ServersApi::serverIpmiPowerPost(
    const int32_t& id, const std::string& action, const int32_t& asset) {
    std::string serializedRequestBody;
    std::string path = m_context + "/servers/{id}/ipmi_power";
    std::map<std::string, std::string> headers;
    static const std::vector<std::string> contentTypes{ "multipart/form-data","application/json", };
...
std::string ServersApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void ServersApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void ServersApi::handleStdException(const std::exception&) {
    throw;
}

void ServersApi::handleUncaughtException() {
    throw;
}
