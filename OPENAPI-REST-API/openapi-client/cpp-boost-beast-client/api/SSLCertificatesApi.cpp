...
SSLCertificatesApiException::SSLCertificatesApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status SSLCertificatesApiException::getStatus() const
{
    return m_status;
}

const char* SSLCertificatesApiException::what() const noexcept
{
    return m_what.c_str();
}

std::shared_ptr<ServiceOrderPostResponse>
SSLCertificatesApi::addSsl(
    const std::shared_ptr<SslOrderRequest>& sslOrderRequest) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ssl/order";
    ...
    if (isJsonContentType(requestContentType)) {
        serializedRequestBody = boost::json::serialize(toRequestJsonValue(sslOrderRequest));
    } else {
    ...
    if (statusCode == boost::beast::http::status(401)) {
        throw SSLCertificatesApiException(statusCode, "Unauthorized");
    }
    throw SSLCertificatesApiException(statusCode, "Unexpected HTTP status code");
}
boost::json::value
SSLCertificatesApi::getNewSsl(
    ) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ssl/order";
    std::map<std::string, std::string> headers;
...
boost::json::value
SSLCertificatesApi::getSslInfo(
    const int32_t& id) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ssl/{id}";
    std::map<std::string, std::string> headers;
    // path params
...
std::shared_ptr<SslCancel_200_response>
SSLCertificatesApi::sslCancel(
    const int32_t& id) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ssl/{id}";
    std::map<std::string, std::string> headers;
    ...
    std::shared_ptr<SslCancel_200_response> deserializedResponse = std::make_shared<SslCancel_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<SslCancel_200_response>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
SSLCertificatesApi::updateSslInfo(
    const std::string& id) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ssl/{id}";
    std::map<std::string, std::string> headers;
...
SSLCertificatesApi::getSslInvoices(
    const int32_t& id) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ssl/{id}/invoices";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "id", id);
...
void
SSLCertificatesApi::getSslList(
    ) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ssl";
    std::map<std::string, std::string> headers;
...
std::shared_ptr<SuccessTextResponse>
SSLCertificatesApi::getSslWelcomeEmail(
    const int32_t& id) {
    std::string serializedRequestBody;
    std::string path = m_context + "/ssl/{id}/welcome_email";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "id", id);
...
std::string SSLCertificatesApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void SSLCertificatesApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void SSLCertificatesApi::handleStdException(const std::exception&) {
    throw;
}

void SSLCertificatesApi::handleUncaughtException() {
    throw;
}
