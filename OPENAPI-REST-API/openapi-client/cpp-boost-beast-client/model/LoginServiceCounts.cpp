...
LoginServiceCounts::LoginServiceCounts(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string LoginServiceCounts::toJsonString(bool prettyJson /* = false */) const
{
...
void LoginServiceCounts::fromJsonString(std::string const& jsonString)
{
    fromJsonValue(boost::json::parse(jsonString));
}

boost::json::value LoginServiceCounts::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void LoginServiceCounts::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for LoginServiceCounts");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object LoginServiceCounts::toJsonObject_internal() const
{
    boost::json::object object;
        object["vps"] = JsonValueConverter<int32_t>::toJsonValue(getVps());
        object["websites"] = JsonValueConverter<int32_t>::toJsonValue(getWebsites());
        object["servers"] = JsonValueConverter<int32_t>::toJsonValue(getServers());
    return object;
}

void LoginServiceCounts::fromJsonObject_internal(boost::json::object const& object)
{
    {
        const auto VpsIt = object.find("vps");
        if (VpsIt != object.end()) {
            setVps(JsonValueConverter<int32_t>::fromJsonValue(VpsIt->value()));
        }
    }
    {
        const auto WebsitesIt = object.find("websites");
        if (WebsitesIt != object.end()) {
            setWebsites(JsonValueConverter<int32_t>::fromJsonValue(WebsitesIt->value()));
        }
    }
    {
        const auto ServersIt = object.find("servers");
        if (ServersIt != object.end()) {
            setServers(JsonValueConverter<int32_t>::fromJsonValue(ServersIt->value()));
        }
    }
}
...
std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<LoginServiceCounts>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<LoginServiceCounts>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<LoginServiceCounts>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<LoginServiceCounts>>>::fromJsonValue(boost::json::parse(json));
}
