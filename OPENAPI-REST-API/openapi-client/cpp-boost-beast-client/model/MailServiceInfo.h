/**
 * InterServer Management API
 * # Overview  The InterServer Management API provides programmatic access to manage your InterServer services. Use this REST API to automate provisioning, configuration, and billing operations across your account.  The API covers the following service categories: - [Domains](https://www.interserver.net/domains/) — registration, transfers, and DNS management - [Web Hosting](https://www.interserver.net/hosting/) — shared and reseller hosting - [VPS Hosting](https://www.interserver.net/vps/) — virtual private servers - [Dedicated Servers](https://www.interserver.net/dedicated/) — bare metal and [Rapid Deploy Servers](https://www.interserver.net/dedicated/rapid-deploy.html) - [Backups](https://www.interserver.net/storage/) — storage and backup services - Licenses — control panel and software licenses - [Mail](https://www.mail.baby/) — mail delivery services - SSL — certificate provisioning - Billing — invoices, payment methods, and account management  For interactive testing, see the [API documentation](/api-docs/).  # Authentication  Most endpoints require authentication. Two methods are supported:  ## API Key (Preferred)  Generate an API key from the [Account Security](https://my.interserver.net/account_security) page on [my.interserver.net](https://my.interserver.net/). Pass it in the `X-API-KEY` request header:  ``` X-API-KEY: your-api-key-here ```  ## Session-Based Authentication  Alternatively, authenticate by creating a session:  1. **Log in** — Send a `POST` request to `/login` with your account credentials. The response includes a session identifier. 2. **Pass the session ID** — Include the session identifier in the `sessionid` header on subsequent requests:  ``` sessionid: your-session-id-here ```  API key authentication is recommended for most integrations as it does not expire and avoids the overhead of session management.  
 *
 * The version of the OpenAPI document: 0.9.0
 * Contact: support@interserver.net
 *
 * NOTE: This class is auto generated by OpenAPI-Generator 7.24.0.
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * MailServiceInfo.h
 *
 * 
 */

#ifndef BOOST_BEAST_OPENAPI_CLIENT_MailServiceInfo_MODEL_H_
#define BOOST_BEAST_OPENAPI_CLIENT_MailServiceInfo_MODEL_H_

#include <string>
#include <vector>
#include <memory>
#include <boost/json.hpp>

namespace org {
namespace openapitools {
namespace client {
namespace model {



/// <summary>
/// 
/// </summary>
class  MailServiceInfo
{
public:
    MailServiceInfo() = default;
    explicit MailServiceInfo(boost::json::value const& value);
    virtual ~MailServiceInfo() = default;

    std::string toJsonString(bool prettyJson = false) const;
    void fromJsonString(std::string const& jsonString);
    boost::json::value toJsonValue() const;
    void fromJsonValue(boost::json::value const& value);

    /////////////////////////////////////////////
    /// MailServiceInfo members

    /// <summary>
    /// The ID of the mail service.
    /// </summary>
    std::string getMailId() const;
    void setMailId(std::string value);

    /// <summary>
    /// The type of mail service.
    /// </summary>
    std::string getMailType() const;
    void setMailType(std::string value);

    /// <summary>
    /// The currency of the mail service.
    /// </summary>
    std::string getMailCurrency() const;
    void setMailCurrency(std::string value);

    /// <summary>
    /// The order date of the mail service.
    /// </summary>
    std::string getMailOrderDate() const;
    void setMailOrderDate(std::string value);

    /// <summary>
    /// The customer ID associated with the mail service.
    /// </summary>
    std::string getMailCustid() const;
    void setMailCustid(std::string value);

    /// <summary>
    /// The mail quota for the service.
    /// </summary>
    std::string getMailQuota() const;
    void setMailQuota(std::string value);

    /// <summary>
    /// The status of the mail service.
    /// </summary>
    std::string getMailStatus() const;
    void setMailStatus(std::string value);

    /// <summary>
    /// The invoice ID of the mail service.
    /// </summary>
    std::string getMailInvoice() const;
    void setMailInvoice(std::string value);

    /// <summary>
    /// The username associated with the mail service.
    /// </summary>
    std::string getMailUsername() const;
    void setMailUsername(std::string value);

    /// <summary>
    /// The IP address associated with the mail service.
    /// </summary>
    std::string getMailIp() const;
    void setMailIp(std::string value);

    /// <summary>
    /// The coupon associated with the mail service.
    /// </summary>
    std::string getMailCoupon() const;
    void setMailCoupon(std::string value);

    /// <summary>
    /// Additional information for the mail service.
    /// </summary>
    std::string getMailExtra() const;
    void setMailExtra(std::string value);

    /// <summary>
    /// The server status of the mail service.
    /// </summary>
    std::string getMailServerStatus() const;
    void setMailServerStatus(std::string value);

    /// <summary>
    /// Additional comments for the mail service.
    /// </summary>
    std::string getMailComment() const;
    void setMailComment(std::string value);

protected:
    //////////////////////////////////////
    // Override these for customization //
    //////////////////////////////////////

    virtual boost::json::object toJsonObject_internal() const;
    virtual void fromJsonObject_internal(boost::json::object const& object);


protected:
    std::string m_Mail_id = "";
    std::string m_Mail_type = "";
    std::string m_Mail_currency = "";
    std::string m_Mail_order_date = "";
    std::string m_Mail_custid = "";
    std::string m_Mail_quota = "";
    std::string m_Mail_status = "";
    std::string m_Mail_invoice = "";
    std::string m_Mail_username = "";
    bool m_Mail_usernameIsSet = false;
    std::string m_Mail_ip = "";
    bool m_Mail_ipIsSet = false;
    std::string m_Mail_coupon = "";
    bool m_Mail_couponIsSet = false;
    std::string m_Mail_extra = "";
    bool m_Mail_extraIsSet = false;
    std::string m_Mail_server_status = "";
    bool m_Mail_server_statusIsSet = false;
    std::string m_Mail_comment = "";
    bool m_Mail_commentIsSet = false;
};

std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<MailServiceInfo>>& data);

void createModelVectorFromJsonString(std::vector<std::shared_ptr<MailServiceInfo>>& vec, const std::string& json);

}
}
}
}


#endif /* BOOST_BEAST_OPENAPI_CLIENT_MailServiceInfo_MODEL_H_ */
