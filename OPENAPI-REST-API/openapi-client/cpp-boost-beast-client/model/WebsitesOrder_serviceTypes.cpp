...
WebsitesOrder_serviceTypes::WebsitesOrder_serviceTypes(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string WebsitesOrder_serviceTypes::toJsonString(bool prettyJson /* = false */) const
{
...
void WebsitesOrder_serviceTypes::fromJsonString(std::string const& jsonString)
{
    fromJsonValue(boost::json::parse(jsonString));
}

boost::json::value WebsitesOrder_serviceTypes::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void WebsitesOrder_serviceTypes::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for WebsitesOrder_serviceTypes");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object WebsitesOrder_serviceTypes::toJsonObject_internal() const
{
    boost::json::object object;
        object["11447"] = JsonValueConverter<std::shared_ptr<WebsitesOrderServiceTypes>>::toJsonValue(getR11447());
    return object;
}

void WebsitesOrder_serviceTypes::fromJsonObject_internal(boost::json::object const& object)
{
    {
        const auto r_11447It = object.find("11447");
        if (r_11447It != object.end()) {
            setR11447(JsonValueConverter<std::shared_ptr<WebsitesOrderServiceTypes>>::fromJsonValue(r_11447It->value()));
        }
    }
}

std::shared_ptr<WebsitesOrderServiceTypes> WebsitesOrder_serviceTypes::getR11447() const
{
    return m_r_11447;
}

void WebsitesOrder_serviceTypes::setR11447(std::shared_ptr<WebsitesOrderServiceTypes> value)
{
    
    m_r_11447 = std::move(value);
}

std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<WebsitesOrder_serviceTypes>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<WebsitesOrder_serviceTypes>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<WebsitesOrder_serviceTypes>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<WebsitesOrder_serviceTypes>>>::fromJsonValue(boost::json::parse(json));
}
