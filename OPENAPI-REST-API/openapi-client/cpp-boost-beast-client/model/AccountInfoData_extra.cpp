...
AccountInfoData_extra::AccountInfoData_extra(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string AccountInfoData_extra::toJsonString(bool prettyJson /* = false */) const
{
...
void AccountInfoData_extra::fromJsonString(std::string const& jsonString)
{
    fromJsonValue(boost::json::parse(jsonString));
}

boost::json::value AccountInfoData_extra::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void AccountInfoData_extra::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for AccountInfoData_extra");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object AccountInfoData_extra::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_Private_whoisIsSet) {
            object["private_whois"] = JsonValueConverter<std::string>::toJsonValue(getPrivateWhois());
        }
    return object;
}

void AccountInfoData_extra::fromJsonObject_internal(boost::json::object const& object)
{
    m_Private_whoisIsSet = false;
    {
        const auto Private_whoisIt = object.find("private_whois");
        if (Private_whoisIt != object.end()) {
            setPrivateWhois(JsonValueConverter<std::string>::fromJsonValue(Private_whoisIt->value()));
        }
    }
}

std::string AccountInfoData_extra::getPrivateWhois() const
{
    return m_Private_whois;
}

void AccountInfoData_extra::setPrivateWhois(std::string value)
{
    
    m_Private_whois = std::move(value);
    m_Private_whoisIsSet = true;
}

std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<AccountInfoData_extra>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<AccountInfoData_extra>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<AccountInfoData_extra>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<AccountInfoData_extra>>>::fromJsonValue(boost::json::parse(json));
}
