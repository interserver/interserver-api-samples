...
VpsServiceExtra::VpsServiceExtra(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string VpsServiceExtra::toJsonString(bool prettyJson /* = false */) const
{
...
void VpsServiceExtra::fromJsonString(std::string const& jsonString)
{
    fromJsonValue(boost::json::parse(jsonString));
}

boost::json::value VpsServiceExtra::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void VpsServiceExtra::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for VpsServiceExtra");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object VpsServiceExtra::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_SpiceIsSet) {
            object["spice"] = JsonValueConverter<int32_t>::toJsonValue(getSpice());
        }
        if (m_SnapshotsIsSet) {
            object["snapshots"] = JsonValueConverter<std::vector<std::shared_ptr<VpsSnapshot>>>::toJsonValue(getSnapshots());
        }
    return object;
}

void VpsServiceExtra::fromJsonObject_internal(boost::json::object const& object)
{
    m_SpiceIsSet = false;
    m_SnapshotsIsSet = false;
    {
        const auto SpiceIt = object.find("spice");
        if (SpiceIt != object.end()) {
            setSpice(JsonValueConverter<int32_t>::fromJsonValue(SpiceIt->value()));
        }
    }
    {
        const auto SnapshotsIt = object.find("snapshots");
        if (SnapshotsIt != object.end()) {
            setSnapshots(JsonValueConverter<std::vector<std::shared_ptr<VpsSnapshot>>>::fromJsonValue(SnapshotsIt->value()));
        }
    }
}
...
void VpsServiceExtra::setSnapshots(std::vector<std::shared_ptr<VpsSnapshot>> value)
{
    
    m_Snapshots = std::move(value);
    m_SnapshotsIsSet = true;
}

std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<VpsServiceExtra>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<VpsServiceExtra>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<VpsServiceExtra>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<VpsServiceExtra>>>::fromJsonValue(boost::json::parse(json));
}
