...
QueueResponse::QueueResponse(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string QueueResponse::toJsonString(bool prettyJson /* = false */) const
{
...
void QueueResponse::fromJsonString(std::string const& jsonString)
{
    fromJsonValue(boost::json::parse(jsonString));
}

boost::json::value QueueResponse::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void QueueResponse::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for QueueResponse");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object QueueResponse::toJsonObject_internal() const
{
...
void QueueResponse::fromJsonObject_internal(boost::json::object const& object)
{
    {
...
std::string QueueResponse::getText() const
{
    return m_Text;
}

void QueueResponse::setText(std::string value)
{
    
    m_Text = std::move(value);
}
int32_t QueueResponse::getQueueId() const
{
    return m_QueueId;
}

void QueueResponse::setQueueId(int32_t value)
{
    
    m_QueueId = std::move(value);
}

std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<QueueResponse>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<QueueResponse>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<QueueResponse>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<QueueResponse>>>::fromJsonValue(boost::json::parse(json));
}
