...
VpsOrder_locationNames::VpsOrder_locationNames(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string VpsOrder_locationNames::toJsonString(bool prettyJson /* = false */) const
{
...
void VpsOrder_locationNames::fromJsonString(std::string const& jsonString)
{
    fromJsonValue(boost::json::parse(jsonString));
}

boost::json::value VpsOrder_locationNames::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void VpsOrder_locationNames::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for VpsOrder_locationNames");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object VpsOrder_locationNames::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_r_3IsSet) {
            object["3"] = JsonValueConverter<std::string>::toJsonValue(getR3());
        }
    return object;
}

void VpsOrder_locationNames::fromJsonObject_internal(boost::json::object const& object)
{
    m_r_3IsSet = false;
    {
        const auto r_3It = object.find("3");
        if (r_3It != object.end()) {
            setR3(JsonValueConverter<std::string>::fromJsonValue(r_3It->value()));
        }
    }
}

std::string VpsOrder_locationNames::getR3() const
{
    return m_r_3;
}

void VpsOrder_locationNames::setR3(std::string value)
{
    
    m_r_3 = std::move(value);
    m_r_3IsSet = true;
}

std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<VpsOrder_locationNames>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<VpsOrder_locationNames>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<VpsOrder_locationNames>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<VpsOrder_locationNames>>>::fromJsonValue(boost::json::parse(json));
}
