...
Server_locations::Server_locations(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string Server_locations::toJsonString(bool prettyJson /* = false */) const
{
...
void Server_locations::fromJsonString(std::string const& jsonString)
{
    fromJsonValue(boost::json::parse(jsonString));
}

boost::json::value Server_locations::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void Server_locations::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for Server_locations");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object Server_locations::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_r_1IsSet) {
            object["1"] = JsonValueConverter<std::shared_ptr<ServerLocation1>>::toJsonValue(getR1());
        }
    return object;
}

void Server_locations::fromJsonObject_internal(boost::json::object const& object)
{
    m_r_1IsSet = false;
    {
        const auto r_1It = object.find("1");
        if (r_1It != object.end()) {
            setR1(JsonValueConverter<std::shared_ptr<ServerLocation1>>::fromJsonValue(r_1It->value()));
        }
    }
}

std::shared_ptr<ServerLocation1> Server_locations::getR1() const
{
    return m_r_1;
}

void Server_locations::setR1(std::shared_ptr<ServerLocation1> value)
{
...
std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<Server_locations>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<Server_locations>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<Server_locations>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<Server_locations>>>::fromJsonValue(boost::json::parse(json));
}
