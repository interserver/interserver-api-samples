...
ServerExtraInfoTables::ServerExtraInfoTables(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string ServerExtraInfoTables::toJsonString(bool prettyJson /* = false */) const
{
...
void ServerExtraInfoTables::fromJsonString(std::string const& jsonString)
{
    fromJsonValue(boost::json::parse(jsonString));
}

boost::json::value ServerExtraInfoTables::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void ServerExtraInfoTables::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for ServerExtraInfoTables");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object ServerExtraInfoTables::toJsonObject_internal() const
{
    boost::json::object object;
        object["assets"] = JsonValueConverter<std::shared_ptr<ServerAssets>>::toJsonValue(getAssets());
    return object;
}

void ServerExtraInfoTables::fromJsonObject_internal(boost::json::object const& object)
{
    {
        const auto AssetsIt = object.find("assets");
        if (AssetsIt != object.end()) {
            setAssets(JsonValueConverter<std::shared_ptr<ServerAssets>>::fromJsonValue(AssetsIt->value()));
        }
    }
}

std::shared_ptr<ServerAssets> ServerExtraInfoTables::getAssets() const
{
    return m_Assets;
}

void ServerExtraInfoTables::setAssets(std::shared_ptr<ServerAssets> value)
{
    
    m_Assets = std::move(value);
}

std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<ServerExtraInfoTables>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<ServerExtraInfoTables>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<ServerExtraInfoTables>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<ServerExtraInfoTables>>>::fromJsonValue(boost::json::parse(json));
}
