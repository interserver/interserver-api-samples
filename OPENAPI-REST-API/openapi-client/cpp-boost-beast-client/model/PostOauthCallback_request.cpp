...
PostOauthCallback_request::PostOauthCallback_request(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string PostOauthCallback_request::toJsonString(bool prettyJson /* = false */) const
{
...
void PostOauthCallback_request::fromJsonString(std::string const& jsonString)
{
    fromJsonValue(boost::json::parse(jsonString));
}

boost::json::value PostOauthCallback_request::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void PostOauthCallback_request::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for PostOauthCallback_request");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object PostOauthCallback_request::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_ProviderIsSet) {
            object["provider"] = JsonValueConverter<std::string>::toJsonValue(getProvider());
        }
    return object;
}

void PostOauthCallback_request::fromJsonObject_internal(boost::json::object const& object)
{
    m_ProviderIsSet = false;
    {
        const auto ProviderIt = object.find("provider");
        if (ProviderIt != object.end()) {
            setProvider(JsonValueConverter<std::string>::fromJsonValue(ProviderIt->value()));
        }
    }
}

std::string PostOauthCallback_request::getProvider() const
{
    return m_Provider;
}

void PostOauthCallback_request::setProvider(std::string value)
{
    
    m_Provider = std::move(value);
    m_ProviderIsSet = true;
}

std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<PostOauthCallback_request>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<PostOauthCallback_request>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<PostOauthCallback_request>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<PostOauthCallback_request>>>::fromJsonValue(boost::json::parse(json));
}
