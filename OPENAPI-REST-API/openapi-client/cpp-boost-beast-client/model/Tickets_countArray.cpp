...
Tickets_countArray::Tickets_countArray(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string Tickets_countArray::toJsonString(bool prettyJson /* = false */) const
{
...
void Tickets_countArray::fromJsonString(std::string const& jsonString)
{
    fromJsonValue(boost::json::parse(jsonString));
}

boost::json::value Tickets_countArray::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void Tickets_countArray::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for Tickets_countArray");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object Tickets_countArray::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_OpenIsSet) {
            object["Open"] = JsonValueConverter<int32_t>::toJsonValue(getOpen());
        }
        if (m_On_HoldIsSet) {
            object["On Hold"] = JsonValueConverter<int32_t>::toJsonValue(getOnHold());
        }
        if (m_ClosedIsSet) {
            object["Closed"] = JsonValueConverter<int32_t>::toJsonValue(getClosed());
        }
    return object;
}

void Tickets_countArray::fromJsonObject_internal(boost::json::object const& object)
{
    m_OpenIsSet = false;
    m_On_HoldIsSet = false;
    m_ClosedIsSet = false;
    {
        const auto OpenIt = object.find("Open");
        if (OpenIt != object.end()) {
            setOpen(JsonValueConverter<int32_t>::fromJsonValue(OpenIt->value()));
        }
    }
    {
        const auto On_HoldIt = object.find("On Hold");
        if (On_HoldIt != object.end()) {
            setOnHold(JsonValueConverter<int32_t>::fromJsonValue(On_HoldIt->value()));
        }
    }
    {
        const auto ClosedIt = object.find("Closed");
        if (ClosedIt != object.end()) {
            setClosed(JsonValueConverter<int32_t>::fromJsonValue(ClosedIt->value()));
        }
    }
}
...
void Tickets_countArray::setClosed(int32_t value)
{
    ...
    m_Closed = std::move(value);
    m_ClosedIsSet = true;
}

std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<Tickets_countArray>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<Tickets_countArray>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<Tickets_countArray>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<Tickets_countArray>>>::fromJsonValue(boost::json::parse(json));
}
