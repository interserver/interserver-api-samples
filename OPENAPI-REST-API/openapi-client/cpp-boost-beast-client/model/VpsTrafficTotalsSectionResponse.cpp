...
VpsTrafficTotalsSectionResponse::VpsTrafficTotalsSectionResponse(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string VpsTrafficTotalsSectionResponse::toJsonString(bool prettyJson /* = false */) const
{
...
void VpsTrafficTotalsSectionResponse::fromJsonString(std::string const& jsonString)
{
    fromJsonValue(boost::json::parse(jsonString));
}

boost::json::value VpsTrafficTotalsSectionResponse::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void VpsTrafficTotalsSectionResponse::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for VpsTrafficTotalsSectionResponse");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object VpsTrafficTotalsSectionResponse::toJsonObject_internal() const
{
    boost::json::object object;
        object["in"] = JsonValueConverter<int64_t>::toJsonValue(getIn());
        object["out"] = JsonValueConverter<int64_t>::toJsonValue(getOut());
    return object;
}

void VpsTrafficTotalsSectionResponse::fromJsonObject_internal(boost::json::object const& object)
{
    {
        const auto InIt = object.find("in");
        if (InIt != object.end()) {
            setIn(JsonValueConverter<int64_t>::fromJsonValue(InIt->value()));
        }
    }
    {
        const auto OutIt = object.find("out");
        if (OutIt != object.end()) {
            setOut(JsonValueConverter<int64_t>::fromJsonValue(OutIt->value()));
        }
    }
}

int64_t VpsTrafficTotalsSectionResponse::getIn() const
{
    return m_In;
}

void VpsTrafficTotalsSectionResponse::setIn(int64_t value)
{
    
    m_In = std::move(value);
}
int64_t VpsTrafficTotalsSectionResponse::getOut() const
{
    return m_Out;
}

void VpsTrafficTotalsSectionResponse::setOut(int64_t value)
{
    
    m_Out = std::move(value);
}

std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<VpsTrafficTotalsSectionResponse>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<VpsTrafficTotalsSectionResponse>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<VpsTrafficTotalsSectionResponse>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<VpsTrafficTotalsSectionResponse>>>::fromJsonValue(boost::json::parse(json));
}
