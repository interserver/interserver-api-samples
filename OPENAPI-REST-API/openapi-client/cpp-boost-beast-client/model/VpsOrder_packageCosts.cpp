...
VpsOrder_packageCosts::VpsOrder_packageCosts(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string VpsOrder_packageCosts::toJsonString(bool prettyJson /* = false */) const
{
...
void VpsOrder_packageCosts::fromJsonString(std::string const& jsonString)
{
    fromJsonValue(boost::json::parse(jsonString));
}

boost::json::value VpsOrder_packageCosts::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void VpsOrder_packageCosts::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for VpsOrder_packageCosts");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object VpsOrder_packageCosts::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_r_57IsSet) {
            object["57"] = JsonValueConverter<double>::toJsonValue(getR57());
        }
    return object;
}

void VpsOrder_packageCosts::fromJsonObject_internal(boost::json::object const& object)
{
    m_r_57IsSet = false;
    {
        const auto r_57It = object.find("57");
        if (r_57It != object.end()) {
            setR57(JsonValueConverter<double>::fromJsonValue(r_57It->value()));
        }
    }
}

double VpsOrder_packageCosts::getR57() const
{
    return m_r_57;
}

void VpsOrder_packageCosts::setR57(double value)
{
    
    m_r_57 = std::move(value);
    m_r_57IsSet = true;
}

std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<VpsOrder_packageCosts>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<VpsOrder_packageCosts>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<VpsOrder_packageCosts>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<VpsOrder_packageCosts>>>::fromJsonValue(boost::json::parse(json));
}
