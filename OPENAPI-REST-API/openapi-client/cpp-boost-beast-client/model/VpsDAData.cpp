...
VpsDAData::VpsDAData(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string VpsDAData::toJsonString(bool prettyJson /* = false */) const
{
...
void VpsDAData::fromJsonString(std::string const& jsonString)
{
    fromJsonValue(boost::json::parse(jsonString));
}

boost::json::value VpsDAData::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void VpsDAData::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for VpsDAData");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object VpsDAData::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_FreeIsSet) {
            object["free"] = JsonValueConverter<std::shared_ptr<VpsDALicense>>::toJsonValue(getFree());
        }
    return object;
}

void VpsDAData::fromJsonObject_internal(boost::json::object const& object)
{
    m_FreeIsSet = false;
    {
        const auto FreeIt = object.find("free");
        if (FreeIt != object.end()) {
            setFree(JsonValueConverter<std::shared_ptr<VpsDALicense>>::fromJsonValue(FreeIt->value()));
        }
    }
}

std::shared_ptr<VpsDALicense> VpsDAData::getFree() const
{
    return m_Free;
}

void VpsDAData::setFree(std::shared_ptr<VpsDALicense> value)
{
    
    m_Free = std::move(value);
    m_FreeIsSet = true;
}

std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<VpsDAData>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<VpsDAData>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<VpsDAData>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<VpsDAData>>>::fromJsonValue(boost::json::parse(json));
}
