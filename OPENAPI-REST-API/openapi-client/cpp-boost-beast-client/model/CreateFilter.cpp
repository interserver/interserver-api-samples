...
CreateFilter::CreateFilter(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string CreateFilter::toJsonString(bool prettyJson /* = false */) const
{
...
void CreateFilter::fromJsonString(std::string const& jsonString)
{
    fromJsonValue(boost::json::parse(jsonString));
}

boost::json::value CreateFilter::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void CreateFilter::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for CreateFilter");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object CreateFilter::toJsonObject_internal() const
{
    boost::json::object object;
        object["filter_type"] = JsonValueConverter<std::string>::toJsonValue(getFilterType());
        object["port"] = JsonValueConverter<int32_t>::toJsonValue(getPort());
    return object;
}

void CreateFilter::fromJsonObject_internal(boost::json::object const& object)
{
    {
        const auto Filter_typeIt = object.find("filter_type");
        if (Filter_typeIt != object.end()) {
            setFilterType(JsonValueConverter<std::string>::fromJsonValue(Filter_typeIt->value()));
        }
    }
    {
...
std::string CreateFilter::getFilterType() const
{
    return m_Filter_type;
}

void CreateFilter::setFilterType(std::string value)
{
    
    m_Filter_type = std::move(value);
}
int32_t CreateFilter::getPort() const
{
    return m_Port;
}

void CreateFilter::setPort(int32_t value)
{
    
    m_Port = std::move(value);
}

std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<CreateFilter>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<CreateFilter>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<CreateFilter>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<CreateFilter>>>::fromJsonValue(boost::json::parse(json));
}
