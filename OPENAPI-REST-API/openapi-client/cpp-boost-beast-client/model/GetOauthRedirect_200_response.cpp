...
GetOauthRedirect_200_response::GetOauthRedirect_200_response(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string GetOauthRedirect_200_response::toJsonString(bool prettyJson /* = false */) const
{
...
void GetOauthRedirect_200_response::fromJsonString(std::string const& jsonString)
{
    fromJsonValue(boost::json::parse(jsonString));
}

boost::json::value GetOauthRedirect_200_response::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void GetOauthRedirect_200_response::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for GetOauthRedirect_200_response");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object GetOauthRedirect_200_response::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_Redirect_urlIsSet) {
            object["redirect_url"] = JsonValueConverter<std::string>::toJsonValue(getRedirectUrl());
        }
    return object;
}

void GetOauthRedirect_200_response::fromJsonObject_internal(boost::json::object const& object)
{
    m_Redirect_urlIsSet = false;
    {
        const auto Redirect_urlIt = object.find("redirect_url");
        if (Redirect_urlIt != object.end()) {
            setRedirectUrl(JsonValueConverter<std::string>::fromJsonValue(Redirect_urlIt->value()));
        }
    }
}

std::string GetOauthRedirect_200_response::getRedirectUrl() const
{
    return m_Redirect_url;
}

void GetOauthRedirect_200_response::setRedirectUrl(std::string value)
{
    
    m_Redirect_url = std::move(value);
    m_Redirect_urlIsSet = true;
}

std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<GetOauthRedirect_200_response>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<GetOauthRedirect_200_response>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<GetOauthRedirect_200_response>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<GetOauthRedirect_200_response>>>::fromJsonValue(boost::json::parse(json));
}
