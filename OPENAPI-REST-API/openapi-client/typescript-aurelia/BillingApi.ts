...
/**
 * getAffiliateDownload - parameters interface
 */
export interface IGetAffiliateDownloadParams {
  st?: string;
  ex?: 'csv' | 'xls' | 'xlsx' | 'pdf';
  year?: number;
}
...
/**
 * getAffiliateSignups - parameters interface
 */
export interface IGetAffiliateSignupsParams {
  st?: string;
}
...
/**
 * patchBillingCreditCardVerify - parameters interface
 */
export interface IPatchBillingCreditCardVerifyParams {
  id: number;
...
export class BillingApi extends Api {
   ...
   * @param params.st Filter by status.
   * @param params.ex Export format: csv, xls, xlsx, or pdf. Defaults to csv.
   * @param params.year Year to filter the report. Defaults to the current year.
   */
  async getAffiliateDownload(params: IGetAffiliateDownloadParams): Promise<any> {
    // Verify required parameters are set

    // Create URL to call
    const url = `${this.basePath}/affiliate/download`;
   ...
   * @param params.st Filter signups by status. Use &#x60;default&#x60; to show all or pass a specific status value to narrow results.
   */
  async getAffiliateSignups(params: IGetAffiliateSignupsParams): Promise<GetAffiliateSignups200Response> {
    // Verify required parameters are set

    // Create URL to call
    const url = `${this.basePath}/affiliate/signups`;
  ...
  async initiatePayment(params: IInitiatePaymentParams): Promise<InitiatePayment200Response> {
    ...
    // Create URL to call
    const url = `${this.basePath}/billing/pay/{method}/{invoices}`
   ...
   * @param params.id The credit card ID to verify. Use the ID returned from &#x60;POST /billing/creditcards&#x60;.
   * @param params.patchBillingCreditCardVerifyRequest 
   */
  async patchBillingCreditCardVerify(params: IPatchBillingCreditCardVerifyParams): Promise<SuccessTextResponse> {
    // Verify required parameters are set
    this.ensureParamIsSet('patchBillingCreditCardVerify', params, 'id');
    this.ensureParamIsSet('patchBillingCreditCardVerify', params, 'patchBillingCreditCardVerifyRequest');
    ...
    const response = await this.httpClient.createRequest(url)
      ...
      // Encode body parameter
      .withHeader('content-type', 'application/json')
      .withContent(JSON.stringify(params['patchBillingCreditCardVerifyRequest'] || {}))
