...
| [**deleteMailAlert**](MailApi.md#deletemailalertoperation) | **DELETE** /mail/{id}/alerts | Delete a Mail Baby alert by alert_id (hard delete — no recovery) |
...
async function example() {
  ...
  const body = {
    // MailOrderRequest
    mailOrderRequest: ...,
  } satisfies AddMailRequest;

  try {
    const data = await api.addMail(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
import type { DeleteMailAlertOperationRequest } from '';
...
async function example() {
  ...
  const body = {
    ...
    deleteMailAlertRequest: ...,
  } satisfies DeleteMailAlertOperationRequest;
...
async function example() {
  ...
  const body = {
    // MailOrderRequest
    mailOrderRequest: ...,
  } satisfies PutMailRequest;

  try {
    const data = await api.putMail(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
async function example() {
  ...
  const body = {
    // number | The mail service ID. Use `mail_id` from `GET /mail`.
    id: 56,
    // string | The ID of the deny rule to update.
    rule: rule_example,
    // DenyRuleNew
    denyRuleNew: ...,
  } satisfies UpdateRuleRequest;

  try {
    const data = await api.updateRule(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
