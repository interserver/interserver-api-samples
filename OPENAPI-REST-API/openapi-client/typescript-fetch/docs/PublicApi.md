...
| [**patchOauthTwoFactor**](PublicApi.md#patchoauthtwofactoroperation) | **PATCH** /oauth | Submit 2FA code to finish OAuth login when account has 2FA enabled |
| [**pingServer**](PublicApi.md#pingserver) | **GET** /ping | Liveness check — returns the JSON string \&quot;pong\&quot; to confirm API is up |
| [**postOauthCallback**](PublicApi.md#postoauthcallbackoperation) | **POST** /oauth | Complete OAuth login by linking provider to existing or new account |
...
import type { GetAccountCurrenciesRequest } from '';
...
async function example() {
  ...
  try {
    const data = await api.getAccountCurrencies();
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
import type { GetAccountLocalesRequest } from '';
...
async function example() {
  ...
  try {
    const data = await api.getAccountLocales();
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
[**{ [key: string]: GetAccountLocales200ResponseValue; }**](GetAccountLocales200ResponseValue.md)
