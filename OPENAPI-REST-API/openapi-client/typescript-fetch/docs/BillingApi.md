...
| [**patchBillingCreditCardVerify**](BillingApi.md#patchbillingcreditcardverifyoperation) | **PATCH** /billing/creditcards/{id}/verify | Place two micro-charges on the card to start CVV verification (step 1 of 2) |
...
import type { GetAffiliateDownloadRequest } from '';
...
async function example() {
  ...
  const body = {
    ...
    st: st_example,
    // 'csv' | 'xls' | 'xlsx' | 'pdf' | Export format: csv, xls, xlsx, or pdf. Defaults to csv. (optional)
    ex: ex_example,
    // number | Year to filter the report. Defaults to the current year. (optional)
    year: 56,
  } satisfies GetAffiliateDownloadRequest;

  try {
    const data = await api.getAffiliateDownload(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
| **ex** | `csv`, `xls`, `xlsx`, `pdf` | Export format: csv, xls, xlsx, or pdf. Defaults to csv. | [Optional] [Defaults to `undefined`] [Enum: csv, xls, xlsx, pdf] |
...
import type { GetAffiliateSignupsRequest } from '';
...
async function example() {
  ...
  const body = {
    // string | Filter signups by status. Use `default` to show all or pass a specific status value to narrow results. (optional)
    st: st_example,
  } satisfies GetAffiliateSignupsRequest;

  try {
    const data = await api.getAffiliateSignups(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
async function example() {
  ...
  const body = {
    ...
    // string | A comma-separated list of invoice IDs or invoice Tags to pay. These IDs are returned by order endpoints (e.g. `/backups/order`, `/vps/order`) and by `/billing/invoices`. Invoice tags accepted are SERVICE<module><id>, RINV<module><repeat invoice id>, INV<module><invoice id>, PREPAY<prepay id><invoice id>, and <invoice id>.
...
import type { PatchBillingCreditCardVerifyOperationRequest } from '';
...
async function example() {
  ...
  const body = {
    ...
    patchBillingCreditCardVerifyRequest: ...,
  } satisfies PatchBillingCreditCardVerifyOperationRequest;

  try {
    const data = await api.patchBillingCreditCardVerify(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
