...
| [**postWebsiteBuyIp**](WebhostingApi.md#postwebsitebuyipoperation) | **POST** /websites/{id}/buy_ip | Buy an additional IP for the website OR update reverse DNS records |
| [**postWebsiteMigration**](WebhostingApi.md#postwebsitemigrationoperation) | **POST** /websites/{id}/migration | Submit a request for InterServer staff to migrate a website from another host |
...
async function example() {
  ...
  const body = {
    ...
    websiteOrderPostRequest: ...,
  } satisfies AddWebsiteRequest;

  try {
    const data = await api.addWebsite(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
async function example() {
  ...
  const body = {
    ...
    websiteOrderPutRequest: ...,
  } satisfies PutWebsitesRequest;

  try {
    const data = await api.putWebsites(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
