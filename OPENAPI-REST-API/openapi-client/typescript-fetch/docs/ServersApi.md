...
| [**placeBuyNowServer**](ServersApi.md#placebuynowserveroperation) | **POST** /servers/order/buy_now_server | Place a Rapid Deploy / coupon dedicated server order; creates real invoice |
...
async function example() {
  ...
  const body = {
    ...
    serverOrderPostRequest: ...,
  } satisfies AddServerRequest;

  try {
    const data = await api.addServer(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
import type { ServerBulkIpmiPowerGetRequest } from '';
...
async function example() {
  ...
  const body = {
    // string | Comma-separated list of Server IDs to query (e.g. `2313,2314,2315`). May also be passed as repeated `ids[]` query parameters.
    ids: 2313,2314,2315,
  } satisfies ServerBulkIpmiPowerGetRequest;

  try {
    const data = await api.serverBulkIpmiPowerGet(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
