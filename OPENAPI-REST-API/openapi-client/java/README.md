...
```xml
<dependency>
  <groupId>io.interserver</groupId>
  <artifactId>interserver-api-client</artifactId>
...
```groovy
  repositories {
    mavenCentral()     // Needed if the 'interserver-api-client' jar has been published to maven central.
    mavenLocal()       // Needed if the 'interserver-api-client' jar has been published to the local maven repo.
  }

  dependencies {
     implementation "io.interserver:interserver-api-client:1.0.0"
  }
```
...
* `target/interserver-api-client-1.0.0.jar`
* `target/lib/*.jar`
...
import io.interserver.client.model.*;
