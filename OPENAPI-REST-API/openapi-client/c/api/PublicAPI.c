...
PublicAPI_getAccountCurrencies(apiClient_t *apiClient)
{
    ...
    // create the path
    char *localVarPath = strdup("/account/currencies");
...
PublicAPI_getAccountLocales(apiClient_t *apiClient)
{
    ...
    // create the path
    char *localVarPath = strdup("/account/locales");
    ...
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        ...
        if(!cJSON_IsArray(PublicAPIlocalVarJSON)) {
            return 0;//nonprimitive container
        }
        elementToReturn = list_createList();
        cJSON *VarJSON;
        cJSON_ArrayForEach(VarJSON, PublicAPIlocalVarJSON)
        {
